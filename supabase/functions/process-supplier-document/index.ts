  ...
  const [suppliers, profiles] = await Promise.all([
    ...
    loadAllRows((from, to) =>
      admin
        ...
        .in('status', ['candidate', 'verified', 'active'])
...
function tryKnownProfiles(
  ...
  if (matchingSuppliers.length !== 1)
    return {
      ...
      candidate: null,
      candidateTest: null,
      diagnostics: [] as ParserDiagnosis[],
    }
  ...
  let firstCandidateDiagnosis: ParserDiagnosis | null = null
  ...
  for (const profile of profiles) {
    ...
    // A candidate mismatch is validation evidence, not a regression of a
    // trusted parser, so it must not enter the verified-profile repair flow.
    if (profile.status === 'candidate') diagnosis.repairEligible = false
    diagnostics.push(diagnosis)
    if (profile.status === 'candidate' && !firstCandidateDiagnosis)
      firstCandidateDiagnosis = diagnosis
    try {
      if (
  ...
  return {
    ...
    candidateTest: candidate
      ? { profileId: candidate.globalProfileId, reason: null }
      : firstCandidateDiagnosis
        ? {
            profileId: firstCandidateDiagnosis.profileId,
            reason:
              firstCandidateDiagnosis.failures[0] ??
              firstCandidateDiagnosis.classification,
          }
        : null,
    diagnostics,
  }
...
async function processSupplierDocumentRequest(request: Request) {
  ...
  try {
    ...
    const storedSelection = document.extraction_metadata.supplierSelection as
      { kind?: string; supplierId?: string; source?: string } | undefined
    const manualHintId =
      storedSelection?.kind === 'manual_hint'
        ? storedSelection.supplierId
        : null
    const detectedHintId =
      storedSelection?.kind === 'detected_hint'
        ? storedSelection.supplierId
        : null
    const selectedHintId = manualHintId ?? detectedHintId
    ...
    const storedOcr =
      selectedHintId && document.ocr_snapshot
    ...
    const hintedSupplier = selectedHintId
      ? supplierCandidates.find(
          (candidate) => candidate.supplierId === selectedHintId,
        )
      : null
    if (selectedHintId && !hintedSupplier)
      throw new Error('El proveedor seleccionado ya no está disponible.')
    const manualSupplier = manualHintId ? hintedSupplier : null
    const detectedSupplier = detectedHintId ? hintedSupplier : null
    const localSupplier =
    ...
    let candidateProfileValidation: {
      profileId: string
      aiConfirmed: boolean
      reason: string | null
    } | null = null
    if (fixture?.knownProfile) {
      ...
      if (known) {
        ...
        const interpreted = parseSupplierDocumentExtraction(
          groundSupplierExtractionInOcr(
        ...
        if (attempted.candidate) {
          ...
          if (validation.candidate) {
            const { error } = await admin.rpc(
              'record_supplier_parser_candidate_ai_confirmation',
              { p_profile_id: attempted.candidate.globalProfileId },
            )
            if (error) throw error
            extraction = attempted.candidate.extraction
            globalProfileId = attempted.candidate.globalProfileId
            globalSupplierId = attempted.candidate.globalSupplierId
            parserMode = 'deterministic'
          } else {
            extraction = interpreted
            parserMode = 'ai'
          }
        } else {
          candidateProfileValidation = attempted.candidateTest
            ? {
                profileId: attempted.candidateTest.profileId,
                aiConfirmed: false,
                reason: attempted.candidateTest.reason,
              }
            : null
          extraction = interpreted
          parserMode = 'ai'
    ...
    if (
      ...
      try {
        ...
        interpretationRetryError =
          error instanceof Error ? error.message : 'INTERPRETATION_RETRY_FAILED'
      }
    }
    ...
    const supplierResolution = manualSupplier
      ...
      : detectedSupplier
        ? {
            supplierId: detectedSupplier.supplierId,
            confidence: 'high' as const,
            signals: ocrSupplierResolution.signals,
            reasons: ['confirmed_system_detection'],
          }
        : ocrSupplierResolution.confidence === 'high'
          ? ocrSupplierResolution
          : resolveSupplierCandidate(extraction.supplier, supplierCandidates)
    const supplierId =
      hintedSupplier?.supplierId ??
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (body.action === 'resume_with_supplier') {
      ...
      const hintSelection = supplierHintSelection(
        supplier.id,
        (metadata.supplierResolution as { supplierId?: unknown } | undefined)
          ?.supplierId,
      )
      const { data: claimed, error } = await admin
        ...
        .update({
          extraction_metadata: {
            ...
            supplierSelection: hintSelection,
            ...(hintSelection.source === 'system'
              ? {}
              : {
                  learningExcluded: true,
                  learningExclusionReason: 'manual_supplier_selection',
