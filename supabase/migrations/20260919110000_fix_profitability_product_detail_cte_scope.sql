...
-- migration-safety-reason: Preserves the existing RPC signature, permissions and response contract while keeping all CTE references within a single statement.

create or replace function public.crm_product_profitability_detail(
  p_venue_id uuid,
  p_product_id uuid,
...
begin
  ...
  with active_variants as (
    select variant.id, variant.name, variant.price_cents, variant.sort_order,
      variant.is_default, format.name as format_name
    ...
    select variant.*, public.theoretical_variant_cost(v_tenant_id, p_venue_id, variant.id, 1) as cost
    from active_variants variant
  ), historical_lines as (
    select line.id, line.tenant_id, line.variant_id,
      ...
      line.theoretical_cost_cents, line.theoretical_cost_known, line.modifiers
    ...
    select coalesce(lines.variant_id::text, 'unknown') as variant_key, lines.format_name,
      sum(lines.quantity) as units, sum(lines.net_sales_cents) as net_sales_cents,
      sum(lines.gross_sales_cents) as gross_sales_cents, sum(lines.discount_cents) as discounts_cents,
    ...
    select lines.id as line_id, coalesce(lines.variant_id::text, 'unknown') as variant_key,
      lines.format_name,
      coalesce(nullif(component.product_name_snapshot, ''), nullif(component.variant_name_snapshot, ''), 'Sin nombre') as mixer_name,
      lines.quantity * component.quantity as units, lines.net_sales_cents, lines.gross_sales_cents
    from historical_lines lines
    ...
    select lines.id as line_id, coalesce(lines.variant_id::text, 'unknown') as variant_key,
      lines.format_name, coalesce(nullif(modifier.value ->> 'name', ''), 'Mixer') as mixer_name,
      lines.quantity as units, lines.net_sales_cents, lines.gross_sales_cents
    from historical_lines lines
    ...
    where modifier.value ->> 'id' ~* '^mixer:[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
      and not exists (select 1 from persisted_mixers persisted where persisted.line_id = lines.id)
  ), mixer_breakdown as (
    select mixer.variant_key, mixer.format_name, mixer.mixer_name,
      sum(mixer.units) as units, sum(mixer.net_sales_cents) as net_sales_cents,
      sum(mixer.gross_sales_cents) as gross_sales_cents, count(*) as line_count
    from (select * from persisted_mixers union all select * from legacy_mixers) mixer
    group by mixer.variant_key, mixer.format_name, mixer.mixer_name
  ), variant_json as (
    select jsonb_agg(jsonb_build_object(
      'variantId', variant.id, 'variantName', variant.name, 'formatName', variant.format_name,
      'priceCents', variant.price_cents,
      'costCents', case when coalesce((variant.cost ->> 'known')::boolean, false)
        then round(coalesce((variant.cost ->> 'cost')::numeric, 0) * 100)::integer else null end,
      'costKnown', coalesce((variant.cost ->> 'known')::boolean, false),
      'components', coalesce(variant.cost -> 'components', '[]'::jsonb)
    ) order by variant.is_default desc, variant.sort_order, variant.id) as variants
    from current_variants variant
  ), breakdown_json as (
    select jsonb_agg(jsonb_build_object(
      'variantId', breakdown.variant_key, 'formatName', breakdown.format_name,
      'units', breakdown.units, 'netSalesCents', breakdown.net_sales_cents,
      'grossSalesCents', breakdown.gross_sales_cents, 'discountsCents', breakdown.discounts_cents,
      ...
      'knownLines', breakdown.known_lines, 'lineCount', breakdown.line_count,
      'mixers', coalesce((
        select jsonb_agg(jsonb_build_object(
          'name', mixer.mixer_name, 'units', mixer.units,
          'netSalesCents', mixer.net_sales_cents, 'grossSalesCents', mixer.gross_sales_cents,
    ...
    ) order by breakdown.units desc, breakdown.format_name) as breakdown
    ...
    select jsonb_build_object(
      ...
      'variantId', default_variant.id, 'variantName', default_variant.name,
      'priceCents', default_variant.price_cents,
      'costCents', case when coalesce((default_variant.cost ->> 'known')::boolean, false)
        then round(coalesce((default_variant.cost ->> 'cost')::numeric, 0) * 100)::integer else null end,
