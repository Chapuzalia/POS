...
create function public.superadmin_update_fiscal_entity_venues(
  p_tenant_id uuid,
...
begin
  ...
  where tenant_id = p_tenant_id and fiscal_entity_id = p_fiscal_entity_id;

  insert into public.fiscal_entity_venues (tenant_id, fiscal_entity_id, venue_id)
  select p_tenant_id, p_fiscal_entity_id, venue_id
  from unnest(p_venue_ids) as requested(venue_id);
end;
$$;
