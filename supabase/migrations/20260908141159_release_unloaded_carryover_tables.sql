-- "No cargar" parks the order/table relationship as well as the order. This
-- makes the physical table immediately available while retaining the original
-- table IDs for a later automatic recovery.
alter table public.restaurant_order_carryovers
  add column if not exists parked_table_ids uuid[] not null default '{}'::uuid[];

update public.restaurant_order_carryovers transfer set parked_table_ids = coalesce((
  select array_agg(distinct ot.table_id order by ot.table_id)
  from public.order_tables ot
  where ot.order_group_id = transfer.order_group_id
), '{}'::uuid[])
where cardinality(transfer.parked_table_ids) = 0;

-- Repair carryovers unloaded while the previous function was installed. Those
-- rows are pending again, but their table links were accidentally left active.
update public.order_tables ot set released_at = coalesce(
  (transfer.recovery_history -> -1 ->> 'at')::timestamptz,
  clock_timestamp()
)
from public.restaurant_order_carryovers transfer
where ot.order_group_id = transfer.order_group_id
  and ot.released_at is null
  and transfer.recovered_at is null
  and jsonb_array_length(transfer.recovery_history) > 0
  and transfer.recovery_history -> -1 ->> 'event' = 'unloaded';
...
begin
  ...
  for transfer_row in select * from public.restaurant_order_carryovers
    where id = any(p_carryover_ids) order by carried_at, id for update
  loop
    ...
    recovered_tables := transfer_row.parked_table_ids;
    if cardinality(recovered_tables) = 0 then
      select coalesce(array_agg(distinct ot.table_id order by ot.table_id), '{}'::uuid[])
        into recovered_tables from public.order_tables ot
        where ot.order_group_id = transfer_row.order_group_id;
    end if;
    if cardinality(recovered_tables) = 0 then continue; end if;

    -- open_restaurant_order serializes occupancy with the same table locks. A
    -- busy table is skipped so opening the cash session never waits or fails.
    begin
      perform 1 from public.restaurant_tables rt
        where rt.id = any(recovered_tables) order by rt.id for update nowait;
    exception when lock_not_available then
      continue;
    end;
    select count(*) into scoped_table_count from public.restaurant_tables rt
      where rt.id = any(recovered_tables)
        and rt.tenant_id = session_row.tenant_id
        and rt.venue_id = session_row.venue_id
        and (rt.is_active or rt.cash_session_id = transfer_row.from_cash_session_id);
    if scoped_table_count <> cardinality(recovered_tables) or exists (
      select 1 from public.order_tables ot
      where ot.table_id = any(recovered_tables)
        and ot.released_at is null
        and ot.order_group_id <> transfer_row.order_group_id
    ) then continue; end if;

    update public.order_tables set released_at = null
      where order_group_id = transfer_row.order_group_id
        and table_id = any(recovered_tables)
        and (
          released_at is null
          or released_at = (transfer_row.recovery_history -> -1 ->> 'at')::timestamptz
        );
    ...
    update public.restaurant_order_carryovers set
      ...
      parked_table_ids = recovered_tables,
...
begin
  ...
  loop
    ...
    update public.order_tables set released_at = unloaded_at_value
      where order_group_id = transfer_row.order_group_id
        and table_id = any(transfer_row.recovery_table_ids)
    ...
    update public.restaurant_order_carryovers set
      ...
      parked_table_ids = transfer_row.recovery_table_ids,
