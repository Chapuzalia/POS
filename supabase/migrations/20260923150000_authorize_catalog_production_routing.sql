...
-- migration-safety-reason: Preserves the get_catalog signature, grants and response contract while enforcing venue access before extending its production-routing payload.
...
begin
  ...
  return v_catalog || jsonb_build_object(
    ...
    'production_routing', case when v_active_only then jsonb_build_object(
      ...
      'productDestinationRoutes', coalesce((select jsonb_agg(jsonb_build_object('productId', route.product_id, 'destinationId', route.destination_id) order by route.product_id) from public.production_product_routes route join public.production_destinations destination on destination.id = route.destination_id and destination.tenant_id = route.tenant_id and destination.venue_id = route.venue_id and destination.is_active where route.tenant_id = v_tenant_id and route.venue_id = p_venue_id), '[]'::jsonb),
      'categoryDestinationRoutes', coalesce((select jsonb_agg(jsonb_build_object('categoryId', route.category_id, 'destinationId', route.destination_id) order by route.category_id) from public.production_category_routes route join public.production_destinations destination on destination.id = route.destination_id and destination.tenant_id = route.tenant_id and destination.venue_id = route.venue_id and destination.is_active where route.tenant_id = v_tenant_id and route.venue_id = p_venue_id), '[]'::jsonb),
      'loadedAt', now()
    ) else null end
  );
end;
