...
-- migration-safety-reason: Creates new ticket-number routines and removes only their default public/anon grants while preserving authenticated access.
set lock_timeout = '5s';
...
update public.tickets ticket_rows
...
alter table public.tickets add constraint tickets_ticket_number_positive_check
  check (ticket_number > 0) not valid;
...
revoke all on function public.next_ticket_number(uuid, uuid, uuid) from public, anon;
...
$$;
revoke all on function public.assign_ticket_number() from public, anon;
