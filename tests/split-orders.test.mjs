...
test('la interfaz permite cantidades parciales, multiples lineas, destino nuevo o existente y cobro individual', () => {
  ...
  assert.match(modal, /Cobrar selección/)
  assert.match(modal, /submitMove\(true\)/)
  assert.match(app, /return result\.targetOrderId/)
  assert.match(modal, /Buscar productos de la comanda/)
  assert.match(modal, /Marca las unidades que quieras cobrar/)
  assert.match(modal, /line\.modifiers\.map/)
  assert.match(modal, /line\.mixer\?\.name/)
  assert.match(modal, /No hay productos que coincidan con la búsqueda/)
