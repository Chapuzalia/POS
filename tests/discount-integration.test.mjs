...
test('el esquema final conserva snapshots y permite pago nulo sin reescribir el historico', () => {
  assert.match(migration, /discount_name text/)
  assert.match(migration, /discount_amount_cents integer/)
  assert.match(migration, /create table public\.sales[\s\S]*payment_method text,/i)
  assert.match(migration, /payment_method IS NULL\) OR \(payment_method = ANY/i)
  assert.match(migration, /sale_payments_method_check/)
  assert.match(migration, /check \(\(method = ANY \(ARRAY\['cash'::text, 'card'::text\]\)\)\) not valid/i)
...
test('el owner configura el redondeo y el servidor lo recalcula y conserva en el ticket', () => {
  assert.match(roundingMigration, /rounding_increment_cents integer/)
  assert.match(roundingMigration, /discount_rounding_increment_cents integer/)
  assert.match(roundingMigration, /discounts_rounding_increment_cents_check[\s\S]*array\[5, 10, 50, 100\]/i)
