...
test('la migracion es aditiva y crea catalogo, componentes, snapshots, indices y RLS', () => {
  for (const table of ['catalog_tabs', 'catalog_placements', 'selection_groups', 'selection_group_items', 'variant_selection_groups', 'product_modifier_groups', 'ticket_line_components', 'order_line_components']) {
  ...
  for (const column of ['sale_format_id', 'sale_format_name_snapshot', 'category_id_snapshot', 'catalog_tab_id_snapshot']) {
    assert.match(migration, new RegExp(`ticket_lines add column if not exists ${column}`))
  }
  assert.doesNotMatch(migration, /drop table|drop column|truncate\s/i)
  assert.match(migration, /comment on column public\.products\.kind is '@deprecated/)
  assert.match(migration, /save_restaurant_order_lines_v3/)
})
