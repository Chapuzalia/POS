...
test('the app uses a visual-viewport height without disabling accessible scaling', async () => {
  ...
  assert.match(page, /compactMobile=\{props\.context\.deviceMode === 'satellite'\}/)
  assert.match(header, /compactMobile \? 'pt-\[max\(\.5rem,env\(safe-area-inset-top\)\)\]'/)
  assert.match(header, /compactMobile \? 'hidden' : 'contents'/)
...
test('the global hook repairs only installed iOS/iPadOS PWA viewports', async () => {
  ...
  assert.match(hook, /let unzoomedAppHeight = viewport\.height/)
  assert.match(hook, /if \(!pageZoomed && !keyboardOpen\) unzoomedAppHeight = viewport\.height/)
  assert.match(hook, /const appHeight = pageZoomed \? unzoomedAppHeight : viewport\.height/)
