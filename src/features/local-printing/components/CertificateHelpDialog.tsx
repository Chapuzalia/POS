  ...
  return <AppModal label="Ayuda para instalar el certificado" onClose={onClose} placement="bottom">
    <section className="max-h-[100svh] w-full overflow-y-auto overscroll-contain [-webkit-overflow-scrolling:touch] rounded-t-[var(--radius)] bg-[var(--surface)] p-5 text-[var(--foreground)] sm:max-w-xl sm:rounded-[var(--radius)]">
      ...
      <ol className="mt-5 grid list-decimal gap-3 pl-5 text-sm leading-6">
        ...
        <li>Si aparece un error de confianza, instala el certificado raíz proporcionado por el agente.</li>
        <li>Ve a Ajustes &gt; General &gt; Información &gt; Ajustes de confianza de certificados.</li>
        <li>Activa la confianza completa, regresa al TPV y vuelve a probar.</li>
      </ol>
      <a className="mt-5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-[var(--radius)] bg-[var(--accent)] px-4 font-bold text-[var(--accent-foreground)]" href={`${baseUrl}/health`} rel="noreferrer" target="_blank"><ExternalLink className="h-4 w-4" />Abrir agente en una nueva pestaña</a>
    </section>
  </AppModal>
}
