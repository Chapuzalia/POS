...
function DenominationTable({ denominations, loading, showStacker }: { denominations: CashlogyAccounting['denominations'] | null; loading: boolean; showStacker: boolean }) {
  ...
  return <div className="border-t border-[var(--separator)]"><DataTable aria-label="Desglose por denominación" className="w-full min-w-[600px] text-sm" filterPlaceholder="Filtrar denominaciones…"><thead className="bg-[var(--background)] text-left text-xs uppercase text-[var(--muted)]"><tr><th className="px-4 py-3">Tipo</th><th className="px-4 py-3 text-right">Valor</th><th className="px-4 py-3 text-right">Reciclador (uds./importe)</th><th className="px-4 py-3 text-right">Stacker (uds./importe)</th><th className="px-4 py-3 text-right">Total</th></tr></thead><tbody className="divide-y divide-[var(--separator)]">{rows.map((row) => renderDenominationRow(row, showStacker))}</tbody></DataTable></div>
}

function renderDenominationRow(row: CashlogyDenomination & { kind: string }, showStacker: boolean) {
  return <tr key={`${row.kind}-${row.valueCents}`}><td className="px-4 py-3 text-[var(--muted)]">{row.kind}</td><td className="px-4 py-3 text-right font-mono font-bold" data-sort-value={row.valueCents}>{formatMoney(row.valueCents)}</td><td className="px-4 py-3 text-right font-mono" data-sort-value={row.recyclerCount}>{row.recyclerCount}/{formatMoney(row.valueCents * row.recyclerCount)}</td><td className="px-4 py-3 text-right font-mono" data-sort-value={showStacker ? row.stackerCount : undefined}>{showStacker ? `${row.stackerCount}/${formatMoney(row.valueCents * row.stackerCount)}` : '••••'}</td><td className="px-4 py-3 text-right font-mono" data-sort-value={showStacker ? row.valueCents * (row.recyclerCount + row.stackerCount) : undefined}>{showStacker ? formatMoney(row.valueCents * (row.recyclerCount + row.stackerCount)) : '••••'}</td></tr>
}
