...
export function PrintAgentSettings({ canConfigure, canOpenDrawer }: { canConfigure: boolean; canOpenDrawer: boolean }) {
  ...
  return <div className="grid gap-5">
    ...
    <section className="rounded-[var(--radius)] border border-[var(--separator)] bg-[var(--background)] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><Server className="h-5 w-5" /><div><h3 className="font-black">Servidor de impresión</h3><p className="break-all font-mono text-xs text-[var(--muted)]">{agent.baseUrl}</p></div></div><div className="flex flex-wrap gap-2"><Button disabled={busy} onClick={() => void run(agent.checkConnection, 'Servidor de impresión conectado.')} size="sm"><RefreshCw className={`h-4 w-4 ${agent.isCheckingConnection ? 'animate-spin' : ''}`} />Probar conexión</Button><Button disabled={!canConfigure} onClick={() => setWizardOpen(true)} size="sm" variant="primary"><Settings2 className="h-4 w-4" />{agent.hasToken ? 'Cambiar servidor' : 'Configurar'}</Button></div></div>
      ...
      <div className="mt-3 flex flex-wrap gap-2"><Button onClick={() => setCertificateHelpOpen(true)} size="sm"><ExternalLink className="h-4 w-4" />Ayuda certificado</Button><Button onClick={() => void copyDiagnostics()} size="sm"><Clipboard className="h-4 w-4" />Copiar diagnóstico</Button></div>
    </section>
    ...
    <section className="rounded-[var(--radius)] border border-[var(--separator)] bg-[var(--background)] p-4">
      ...
      <label className="mt-3 block"><span className="mb-2 block text-sm font-bold">Código de terminal</span><UiInput className="min-h-12 w-full rounded-[var(--radius)] border border-[var(--field-border)] bg-[var(--field)] px-3 font-mono" disabled={!canConfigure || !agent.cashlogyConfigured} maxLength={64} onChange={(event) => agent.updateCashlogyConfiguration({ terminalCode: event.target.value })} value={agent.cashlogyTerminalCode} /></label>
      {agent.cashlogyConfigured ? <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <Metric label="Estado" tone={agent.cashlogyReady ? 'success' : 'danger'} value={agent.cashlogyReady ? 'Listo' : agent.cashlogyHealth?.sessionState ?? 'Sin consultar'} />
        <Metric label="Modelo" value={agent.cashlogyHealth?.device?.model || '-'} />
        <Metric label="Número de serie" value={agent.cashlogyHealth?.device?.serialNumber || '-'} />
    ...
    <section className="rounded-[var(--radius)] border border-[var(--separator)] bg-[var(--background)] p-4">
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        {[
          ['printCashClosingAutomatically', 'Imprimir automáticamente al cerrar'],
