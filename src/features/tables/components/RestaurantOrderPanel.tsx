...
function OrderLineRow({ discount, isBusy, line, onDecrement, onEdit, onIncrement, onOpenQuantityEditor, onOpenUnitPriceEditor, onRemove, onServeAll, onServeOne, productionState }: Omit<Props, 'order' | 'onServeAllOrder' | 'lineDiscounts' | 'onSendToProduction' | 'onSetQuantity' | 'onSetUnitPrice'> & { discount?: LineDiscountAllocation; line: RestaurantOrderLine; onOpenQuantityEditor: () => void; onOpenUnitPriceEditor: () => void }) {
...
export function RestaurantOrderPanel(props: Props) {
  ...
  const renderLine = (line: RestaurantOrderLine) => <OrderLineRow {...lineProps} discount={lineDiscounts[line.id]} isBusy={isBusy} key={line.id} line={line} onOpenQuantityEditor={() => openQuantityEditor(line)} onOpenUnitPriceEditor={() => openUnitPriceEditor(line)} productionState={productionState} />
