...
export function TableOrderBar({ isBusy, isOnline, onBack, onMove, order, quickSale, saveState, canSell }: Props) {
  return (
    <div className="mx-auto flex w-full max-w-[1600px] items-center gap-3 px-4 pt-3 justify-between">
      ...
      {order ? <>
        <div className="min-w-0 flex flex-col items-center"><strong className="block truncate">{order.tables.map((table) => table.name).join(' + ')}</strong><span className="text-sm text-[var(--muted)]">{order.order.guestCount} comensales</span></div>
