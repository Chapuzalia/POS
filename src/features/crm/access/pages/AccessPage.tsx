...
export function AccessManagementCrm({
  ...
  const [editingDeviceMode, setEditingDeviceMode] =
    useState<DeviceMode>("checkout");
  ...
  const [editingManagerVenueIds, setEditingManagerVenueIds] = useState<
    string[]
  >([]);
  ...
  async function submitDevice(event: FormEvent<HTMLFormElement>) {
    ...
    await runAction(async () => {
      const credentials = await createCrmDevice(
        tenantContext,
        deviceVenueId,
        deviceName,
        deviceMode,
  ...
  function openUserModal() {
    ...
    setUserRole("manager");
    setUserVenueIds(
      data.venues.filter((venue) => venue.isActive).map((venue) => venue.id),
    );
  ...
  async function submitUser(event: FormEvent<HTMLFormElement>) {
    ...
    await runAction(async () => {
      ...
      setData((current) => ({
        ...current,
        users: [...current.users, user].sort((first, second) =>
  ...
  function openManagerVenues(user: CrmAccessData["users"][number]) {
    ...
    setEditingManagerVenueIds(
      user.venueIds.filter((venueId) =>
        data.venues.some((venue) => venue.id === venueId && venue.isActive),
      ),
    );
  }
  ...
  async function saveManagerVenues(event: FormEvent<HTMLFormElement>) {
    ...
    await runAction(async () => {
      const venueIds = await updateManagerVenueAssignments(
        tenantContext,
        editingManagerId,
        editingManagerVenueIds,
      );
      setData((current) => ({
        ...current,
        users: current.users.map((user) =>
          user.id === editingManagerId ? { ...user, venueIds } : user,
        ),
      }));
    ...
    <>
      <div className="!grid !grid-cols-1 !items-start !gap-4 xl:!grid-cols-[340px_minmax(0,1fr)] xl:!gap-6">
        <section className="min-w-0 overflow-hidden rounded-[var(--crm-radius-lg)] border-0 bg-[var(--crm-surface)] text-[var(--crm-text)] shadow-[var(--crm-shadow-card)] !min-w-0 !overflow-hidden !rounded-2xl !border-0 !bg-[var(--crm-surface)] !shadow-[var(--crm-shadow-card)] sm:!rounded-[var(--crm-radius-lg)]">
          ...
          <form
            className="grid gap-3 p-3.5 !grid !gap-3.5 !px-[22px] !pt-5 !pb-[22px]"
            onSubmit={(event) => void submitDevice(event)}
          >
            <Field label="Local">
              <CrmSelect
                disabled={disabled}
            ...
            <Field label="Modo">
              <CrmSelect
                onChange={(value) => setDeviceMode(value as DeviceMode)}
                options={modeOptions}
                value={deviceMode}
              />
            </Field>
            ...
            <UiButton
              ...
              disabled={disabled || !deviceVenueId || !deviceName.trim()}
              type="submit"
            >
        ...
        <section className="min-w-0 overflow-hidden rounded-[var(--crm-radius-lg)] border-0 bg-[var(--crm-surface)] text-[var(--crm-text)] shadow-[var(--crm-shadow-card)] !min-w-0 !overflow-hidden !rounded-2xl !border-0 !bg-[var(--crm-surface)] !shadow-[var(--crm-shadow-card)] sm:!rounded-[var(--crm-radius-lg)]">
          ...
          <div className="!divide-y !divide-[var(--crm-border-subtle)]">
            {data.devices.map((device) => {
              ...
              const mode =
                modeOptions.find((option) => option.value === device.deviceMode)
                  ?.label ?? "Caja";
                ...
                <div className="!px-[18px] !py-3 md:!px-[22px]" key={device.id}>
                  <div className="!grid !min-h-[68px] !items-center !gap-3 md:!grid-cols-[minmax(150px,0.8fr)_minmax(220px,1.2fr)_100px_auto]">
                    <div className="grid min-w-0 gap-[3px] [&_strong]:truncate [&_strong]:text-sm [&_strong]:font-semibold [&_strong]:text-[var(--crm-text)] [&_span]:truncate [&_span]:text-xs [&_span]:font-medium [&_span]:text-[var(--crm-text-muted)]">
                      ...
                      <span>
                        {venue?.name ?? "Local no disponible"} · {mode}
                      </span>
                    </div>
                    <div className="grid min-w-0 gap-[3px] [&_strong]:truncate [&_strong]:text-sm [&_strong]:font-semibold [&_strong]:text-[var(--crm-text)] [&_span]:truncate [&_span]:text-xs [&_span]:font-medium [&_span]:text-[var(--crm-text-muted)]">
                      <strong>
                        {device.account?.email || "Sin credenciales asociadas"}
                      </strong>
                      <span>
                        {device.account
                          ? "Credencial de acceso del dispositivo"
                          : "Requiere revisión"}
                      </span>
                    </div>
                    <div className="!grid !justify-items-start !gap-1">
                      <span
                        className={
                          device.account?.isActive
                            ? "inline-flex min-h-6 w-fit items-center whitespace-nowrap rounded-full px-[9px] text-[11px] font-semibold !inline-flex !min-h-6 !rounded-full !bg-[var(--crm-green-soft)] !px-[9px] !text-[11px] !font-semibold !text-[var(--crm-green)]"
                            : "inline-flex min-h-6 w-fit items-center whitespace-nowrap rounded-full px-[9px] text-[11px] font-semibold !inline-flex !min-h-6 !rounded-full !bg-[var(--crm-surface-soft)] !px-[9px] !text-[11px] !font-semibold !text-[var(--crm-text-secondary)]"
                        }
                      >
                        {device.account?.hasActiveLogin
                          ? "En sesión"
                          : device.account?.isActive
                            ? "Activo"
                            : "Inactivo"}
                    ...
                    <div className="!flex !flex-wrap !items-center !justify-start !gap-2 md:!justify-end">
                      <UiButton
                        ...
                        className="inline-flex min-h-10 w-auto items-center justify-center gap-2 rounded-[var(--crm-radius-sm)] border-0 bg-[var(--crm-blue)] px-3.5 text-[13px] font-semibold leading-none text-white shadow-none transition-[background-color,color,box-shadow,transform] duration-150 hover:bg-[var(--crm-blue-hover)] hover:shadow-[0_8px_20px_rgba(20,120,237,0.22)] !inline-flex !size-10 !items-center !justify-center !rounded-[10px] !bg-[var(--crm-blue)] !text-white"
                        disabled={disabled || !device.account}
                        onClick={() => startEditingDevice(device)}
                        title="Editar nombre, modo o contraseña"
                        type="button"
                      >
                  ...
                  {isEditing ? (
                    <form
                      className="!mt-3 !grid !grid-cols-1 !gap-3 !rounded-[var(--crm-radius-md)] !bg-[var(--crm-surface-soft)] !p-4 md:!grid-cols-3"
                      onSubmit={(event) => void submitDeviceEdit(event)}
                    >
                      <Field label="Nombre">
                        <UiInput
                          className="h-11 min-h-11 w-full rounded-[var(--crm-radius-sm)] border border-transparent bg-[var(--crm-input-bg)] px-3.5 text-[13px] font-medium leading-[1.4] text-[var(--crm-text)] shadow-none outline-none transition-[border-color,box-shadow,background-color] duration-150 placeholder:text-[var(--crm-text-muted)] focus:border-[var(--crm-blue)] focus:shadow-[0_0_0_3px_var(--crm-blue-soft)] [&:is(textarea)]:h-auto [&:is(textarea)]:min-h-[88px] [&:is(textarea)]:resize-y [&:is(textarea)]:py-[11px] !h-11 !w-full !rounded-[10px] !border !border-transparent !bg-[var(--crm-input-bg)] !px-3.5 !text-[13px] !font-medium"
                      ...
                      <Field label="Nueva contraseña (opcional)">
                        <UiInput
                          ...
                          minLength={6}
                          onChange={(event) =>
                            setEditingDevicePassword(event.target.value)
                          }
                          placeholder="6 caracteres"
                          type="text"
                          value={editingDevicePassword}
                        />
                      </Field>
                      <Field label="Modo">
                        <CrmSelect
                          disabled={disabled}
                          onChange={(value) =>
                            setEditingDeviceMode(value as DeviceMode)
                          }
                          options={modeOptions}
                          value={editingDeviceMode}
                        />
                      </Field>
                      <p className="!m-0 !text-xs !text-[var(--crm-text-muted)] md:!col-span-3">
                        Al cambiar el nombre también se actualizará
                        automáticamente el email de acceso.
                      </p>
                      <div className="!flex !gap-2 md:!col-span-3 md:!justify-end">
                        ...
                        <UiButton
                          ...
                          disabled={
                            ...
                            (editingDevicePassword.length > 0 &&
                              editingDevicePassword.length !== 6)
                          }
                          type="submit"
                        >
            ...
            {!data.devices.length ? (
              <EmptyList message="No hay dispositivos registrados." />
            ) : null}
          </div>
        </section>
        ...
        {tenantContext.role === "owner" ? (
          <section className="min-w-0 overflow-hidden rounded-[var(--crm-radius-lg)] border-0 bg-[var(--crm-surface)] text-[var(--crm-text)] shadow-[var(--crm-shadow-card)] !min-w-0 !overflow-hidden !rounded-2xl !border-0 !bg-[var(--crm-surface)] !shadow-[var(--crm-shadow-card)] sm:!rounded-[var(--crm-radius-lg)] xl:!col-span-2">
            <div className="flex items-center justify-between gap-4 border-b border-[var(--crm-border-subtle)] bg-[var(--crm-surface)] p-3 max-[760px]:flex-col max-[760px]:items-stretch !flex !items-center !justify-between !gap-4 !border-b !border-[var(--crm-border-subtle)] !bg-transparent !px-[18px] !py-5 !text-[var(--crm-text)] md:!px-[22px]">
              <div className="min-w-0 [&_h2]:m-0 [&_h2]:text-[17px] [&_h2]:font-bold [&_h2]:tracking-[-0.02em] [&_h2]:text-[var(--crm-text)] [&_p]:mt-1 [&_p]:mb-0 [&_p]:text-xs [&_p]:font-medium [&_p]:text-[var(--crm-text-muted)]">
                <h2>Usuarios con acceso al CRM</h2>
                <p>
                  {data.users.length} usuarios gestores
                </p>
              </div>
              <UiButton
            ...
            <div className="!divide-y !divide-[var(--crm-border-subtle)]">
              {data.users.map((user) => (
                <div
                  className="!grid !min-h-[68px] !items-center !gap-3 !px-[18px] !py-3 sm:!grid-cols-[minmax(0,1fr)_110px_minmax(160px,.8fr)_80px_auto] md:!px-[22px]"
                  key={user.id}
                >
                  <div className="grid min-w-0 gap-[3px] [&_strong]:truncate [&_strong]:text-sm [&_strong]:font-semibold [&_strong]:text-[var(--crm-text)] [&_span]:truncate [&_span]:text-xs [&_span]:font-medium [&_span]:text-[var(--crm-text-muted)]">
                  ...
                  <span className="!text-sm !font-semibold !text-[var(--crm-text-secondary)]">
                  ...
                  <span
                    className={
                      user.isActive
                        ? "inline-flex min-h-6 w-fit items-center whitespace-nowrap rounded-full px-[9px] text-[11px] font-semibold !inline-flex !min-h-6 !w-fit !rounded-full !bg-[var(--crm-green-soft)] !px-[9px] !text-[11px] !font-semibold !text-[var(--crm-green)]"
                        : "inline-flex min-h-6 w-fit items-center whitespace-nowrap rounded-full px-[9px] text-[11px] font-semibold !inline-flex !min-h-6 !w-fit !rounded-full !bg-[var(--crm-surface-soft)] !px-[9px] !text-[11px] !font-semibold !text-[var(--crm-text-secondary)]"
                    }
                  >
                  ...
                  {user.role === "manager" ? (
                    <UiButton
                      aria-label={`Gestionar locales de ${user.email}`}
                      className="!inline-flex !min-h-9 !items-center !gap-2 !rounded-[9px] !border-0 !bg-[var(--crm-surface-soft)] !px-3 !text-xs !font-semibold !text-[var(--crm-text-secondary)]"
                      disabled={disabled}
                      onClick={() => openManagerVenues(user)}
                      type="button"
                    >
                      <MapPin className="!size-4" /> Locales
                    </UiButton>
                  ) : (
                    <span />
                  )}
              ...
              {!data.users.length ? (
                <EmptyList message="No hay usuarios gestores configurados." />
              ) : null}
            </div>
          </section>
        ) : null}
          ...
          <form onSubmit={(event) => void submitUser(event)}>
            <div className="!flex !items-start !justify-between !gap-4 !border-b !border-[var(--crm-border)] !px-5 !py-4">
              <div>
                ...
                <p className="!mt-1 !mb-0 !text-xs !text-[var(--crm-text-muted)]">
                  Crea sus credenciales y asigna el nivel de acceso al negocio.
                </p>
              </div>
              <UiButton
                aria-label="Cerrar"
            ...
            <div className="!grid !gap-4 !px-5 !py-5">
              ...
              <Field label="Rol">
                <CrmSelect
                  disabled={disabled}
                  onChange={(value) =>
                    setUserRole(value as "owner" | "manager")
                  }
                  options={roleOptions}
                  required
                  value={userRole}
              ...
              {userRole === "manager" ? (
                <VenueScopePicker
                  disabled={disabled}
                  onChange={(venueId, checked) =>
                    toggleVenue(venueId, checked, setUserVenueIds)
                  }
                  selectedVenueIds={userVenueIds}
                  venues={data.venues}
                />
              ) : null}
              <p className="m-0 text-xs leading-6 text-[var(--crm-text-muted)]">
                La contraseña debe tener al menos {CRM_USER_PASSWORD_MIN_LENGTH}{" "}
                caracteres.
              </p>
              <div className="!flex !flex-col-reverse !gap-2 sm:!flex-row sm:!justify-end">
                ...
                <UiButton
                  ...
                  disabled={
                    disabled ||
                    !userEmail.trim() ||
                    userPassword.length < CRM_USER_PASSWORD_MIN_LENGTH ||
                    (userRole === "manager" && !userVenueIds.length)
                  }
                  type="submit"
                >
      ...
      {editingManagerId ? (
        ...
        >
          <form onSubmit={(event) => void saveManagerVenues(event)}>
            <div className="!flex !items-start !justify-between !gap-4 !border-b !border-[var(--crm-border)] !px-5 !py-4">
              <div>
                <h2 className="!m-0 !text-lg !font-bold">Locales permitidos</h2>
                <p className="!mt-1 !mb-0 !text-xs !text-[var(--crm-text-muted)]">
                  El manager solo podrá ver y gestionar los locales
                  seleccionados.
                </p>
              </div>
              <UiButton
            ...
            <div className="!grid !gap-4 !px-5 !py-5">
              <VenueScopePicker
                disabled={disabled}
                onChange={(venueId, checked) =>
                  toggleVenue(venueId, checked, setEditingManagerVenueIds)
                }
              ...
              <div className="!flex !justify-end !gap-2">
                ...
                <UiButton
                  ...
                  disabled={disabled || !editingManagerVenueIds.length}
                  type="submit"
                >
      ...
      {generatedCredentials ? (
        ...
        >
          <div className="!flex !items-start !justify-between !gap-4 !border-b !border-[var(--crm-border)] !px-5 !py-4">
            <div>
              <h2 className="!m-0 !text-lg !font-bold">Dispositivo creado</h2>
              <p className="!mt-1 !mb-0 !text-xs !text-[var(--crm-text-muted)]">
                Guarda estas credenciales: la contraseña solo se muestra ahora.
              </p>
            </div>
            <UiButton
              aria-label="Cerrar"
          ...
          <div className="!grid !gap-4 !px-5 !py-5">
            <CredentialRow
              ...
              onCopy={() =>
                void copyCredential(generatedCredentials.email, "Email")
              }
            ...
            <CredentialRow
              ...
              large
              onCopy={() =>
                void copyCredential(generatedCredentials.password, "Contraseña")
              }
...
function CredentialRow({
  ...
  return (
    <div className="!grid !gap-2">
      <span className="!text-[11px] !font-semibold !text-[var(--crm-text-muted)]">
        {label}
      </span>
      <div className="!flex !items-center !gap-2 !rounded-[10px] !bg-[var(--crm-surface-soft)] !p-3">
        <code
          className={
            large
              ? "!min-w-0 !flex-1 !text-lg !font-bold !tracking-[0.08em]"
              : "!min-w-0 !flex-1 !overflow-hidden !text-ellipsis !text-sm !font-semibold"
          }
        >
          {value}
        </code>
        <UiButton
          ...
          className="inline-flex size-9 min-h-9 min-w-9 items-center justify-center gap-2 rounded-[9px] border-0 bg-[var(--crm-surface-soft)] p-0 text-[var(--crm-text-secondary)] shadow-none transition-[background-color,color,transform] duration-150 hover:bg-[var(--crm-surface-hover)] hover:text-[var(--crm-text)] !inline-flex !size-9 !shrink-0 !items-center !justify-center !rounded-[9px] !bg-[var(--crm-input-bg)] !text-[var(--crm-text-secondary)]"
          onClick={onCopy}
          type="button"
        >
...
function VenueScopePicker({
  ...
  const allSelected =
    ...
    activeVenues.every((venue) => selectedVenueIds.includes(venue.id));
  return (
    <fieldset className="!grid !gap-2 !rounded-[10px] !border !border-[var(--crm-border-subtle)] !p-3">
      <legend className="!px-1 !text-xs !font-semibold !text-[var(--crm-text-muted)]">
        Locales permitidos
      </legend>
      <UiCheckbox
        checked={allSelected}
        ...
        onChange={(checked) =>
          activeVenues.forEach((venue) => onChange(venue.id, checked))
        }
      >
        Todos los locales
      </UiCheckbox>
      <div className="!grid !gap-2 sm:!grid-cols-2">
        {activeVenues.map((venue) => (
          <UiCheckbox
            ...
            disabled={disabled}
            key={venue.id}
            onChange={(checked) => onChange(venue.id, checked)}
          >
            {venue.name}
          </UiCheckbox>
        ))}
