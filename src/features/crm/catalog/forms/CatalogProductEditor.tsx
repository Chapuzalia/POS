...
export function CatalogProductEditor({
  ...
  const formatById = useMemo(
    () => new Map(catalog.saleFormats.map((format) => [format.id, format])),
    [catalog.saleFormats],
  );
  ...
  const productPlacements = useMemo(
    () =>
      catalog.placements.filter(
        (placement) => placement.productId === product?.id,
      ),
    [catalog.placements, product?.id],
  );
  const productAssignments = useMemo(
        ...
        .filter(
          (assignment) =>
            ...
            catalog.selectionGroups.some(
              (group) =>
                group.id === assignment.groupId && group.type === "mixer",
            ),
        )
    ...
    [
      catalog.modifierAssignments,
      catalog.selectionAssignments,
      catalog.selectionGroups,
      product?.id,
    ],
  );
  const [name, setName] = useState(product?.name ?? "");
  const [description, setDescription] = useState(product?.description ?? "");
  const [type] = useState<"standard">("standard");
  const [vatMode, setVatMode] = useState<"default" | "custom">(
    product?.vatRate === null || product?.vatRate === undefined
      ? "default"
      : "custom",
  );
  const [vatRate, setVatRate] = useState(
    String(product?.vatRate ?? defaultTaxRate),
  );
  ...
  const [tabId, setTabId] = useState(
    catalog.tabs.find((tab) => tab.active)?.id ?? "",
  );
  const [categoryId, setCategoryId] = useState(
    catalog.categories.find((category) => category.active)?.id ?? "",
  );
  const [internal, setInternal] = useState(false);
  const [initialPlacementFeatured, setInitialPlacementFeatured] =
    useState(false);
  ...
  const [placementTabId, setPlacementTabId] = useState(
    catalog.tabs.find((tab) => tab.active)?.id ?? "",
  );
  ...
  const previewUrl = useMemo(
    () =>
      imageFile
        ...
        : (product?.image?.publicUrl ?? null),
    [imageFile, product?.image?.publicUrl],
  );
  ...
  async function submitGeneral(event: FormEvent) {
    ...
    const parsedVat =
      vatMode === "default" ? null : Number(vatRate.replace(",", "."));
    if (
      parsedVat !== null &&
      (!Number.isFinite(parsedVat) || parsedVat < 0 || parsedVat > 100)
    )
      return setFormError("El IVA debe estar entre 0 y 100.");

    if (product) {
      const saved = await mutate(() =>
        catalogAdminService.updateProduct(catalog.venueId, {
          id: product.id,
          type,
          name: name.trim(),
      ...
      if (saved && imageFile)
        await mutate(() =>
          catalogAdminService.uploadProductImage({
    ...
    const variants = (advanced ? variantDrafts : variantDrafts.slice(0, 1)).map(
      (variant, index) => ({
        id: variant.id,
        formatId: variant.formatId,
    ...
    if (!internal && (!tabId || !categoryId))
      return setFormError(
        "Elige una pestaña y categoría, o marca el producto como interno.",
      );
      ...
      placement: internal
        ...
        : {
            ...
            sortOrder:
              catalog.placements.filter(
                (placement) =>
                  placement.tabId === tabId &&
                  placement.categoryId === categoryId,
              ).length * 10,
          },
    });
    const saved = await mutate(() =>
  ...
  async function addVariant() {
    ...
    await mutate(() =>
      catalogAdminService.createVariant(catalog.venueId, product.id, {
        formatId: format.id,
        name: format.name,
        priceCents: cents(newVariantPrice),
        active: true,
        isDefault: productVariants.length === 0,
        sortOrder: productVariants.length * 10,
      }),
    );
  ...
  function beginVariantEdit(variantId: string) {
    ...
    setEditingVariantPrice(
      (variant.priceCents / 100).toFixed(2).replace(".", ","),
    );
  }
  ...
  async function savePlacement() {
    ...
    const existing = productPlacements.find(
      (placement) => placement.id === placementEditId,
    );
    const saved = existing
      ? await mutate(() =>
          catalogAdminService.updatePlacement(catalog.venueId, {
            ...existing,
      ...
      : await mutate(() =>
          catalogAdminService.createPlacement(catalog.venueId, {
            productId: product.id,
            tabId: placementTabId,
            categoryId: placementCategoryId,
            pinnedVariantId: placementVariantId || null,
  ...
  async function addAssignment() {
    ...
    const optionsCount =
      assignmentDomain === "selection"
        ? catalog.selectionOptions.filter(
            (option) => option.groupId === assignmentGroupId && option.active,
          ).length
        : catalog.modifiers.filter(
            (modifier) =>
              modifier.groupId === assignmentGroupId && modifier.active,
          ).length;
    const validation = validateSelectionCapacity({
      minSelection: assignmentMin,
      maxSelection: assignmentMax,
      required: assignmentMin > 0,
      availableOptions: optionsCount,
    });
      ...
      catalogAdminService.saveAssignment(catalog.venueId, {
        ...
        sortOrder:
          productAssignments.filter(
            (assignment) => assignment.domain === assignmentDomain,
          ).length * 10,
      }),
    );
  }
  ...
  const assignmentGroups =
    assignmentDomain === "selection"
      ? catalog.selectionGroups.filter((group) => group.type === "mixer")
      ...
      <div className="!grid !gap-5 !overflow-y-auto !p-5">
        ...
        >
          ...
          <div className="!grid !gap-3 sm:!grid-cols-2">
            ...
            <Field label="Tipo">
              <CrmSelect
                disabled
                options={[{ label: "Producto estándar", value: "standard" }]}
                value={type}
              />
            </Field>
            <Field className="sm:!col-span-2" label="IVA">
              <div className="!grid !gap-2 sm:!grid-cols-2">
                <CrmSelect
                  ...
                  onChange={(value) => {
                    setVatMode(value as typeof vatMode);
                    markDirty();
                  }}
                  options={[
                    {
                      label: `IVA predeterminado del local (${defaultTaxRate} %)`,
                      value: "default",
                    },
          ...
          <Field label="Descripción">
            <UiTextArea
              className="h-11 min-h-11 w-full rounded-[var(--crm-radius-sm)] border border-transparent bg-[var(--crm-input-bg)] px-3.5 text-[13px] font-medium leading-[1.4] text-[var(--crm-text)] shadow-none outline-none transition-[border-color,box-shadow,background-color] duration-150 placeholder:text-[var(--crm-text-muted)] focus:border-[var(--crm-blue)] focus:shadow-[0_0_0_3px_var(--crm-blue-soft)] [&:is(textarea)]:h-auto [&:is(textarea)]:min-h-[88px] [&:is(textarea)]:resize-y [&:is(textarea)]:py-[11px] !min-h-20"
              onChange={(event) => {
          ...
          <div className="!grid !gap-3 sm:!grid-cols-[110px_1fr] sm:!items-center">
            <div className="grid size-[42px] shrink-0 place-items-center overflow-hidden rounded-xl border-0 bg-[var(--crm-surface-soft)] object-cover text-[var(--crm-text-muted)] !size-24 !overflow-hidden">
              {previewUrl ? (
                <img
            ...
            <div className="!grid !gap-2">
              <label className="inline-flex min-h-10 w-auto items-center justify-center gap-2 rounded-[var(--crm-radius-sm)] border-0 bg-[var(--crm-input-bg)] px-3.5 text-[13px] font-semibold leading-none text-[var(--crm-text-secondary)] shadow-none transition-[background-color,color,box-shadow,transform] duration-150 hover:bg-[var(--crm-surface-hover)] hover:text-[var(--crm-text)] !inline-flex !w-fit !cursor-pointer !items-center !gap-2">
                <ImagePlus className="!size-4" /> Seleccionar imagen
                <UiInput
              ...
              {product?.image ? (
                <UiButton
                  className="inline-flex min-h-10 w-auto items-center justify-center gap-2 rounded-[var(--crm-radius-sm)] border-0 bg-[var(--crm-red-soft)] px-3.5 text-[13px] font-semibold leading-none text-[var(--crm-red)] shadow-none transition-[background-color,color,box-shadow,transform] duration-150 hover:brightness-95 !w-fit"
                  disabled={disabled}
                  onClick={() =>
                    void mutate(() =>
                      catalogAdminService.deleteProductImage(
                        catalog.venueId,
                        product.id,
                      ),
                    )
          ...
          {!product ? (
            <>
              ...
              >
                Configuración avanzada de variantes
              </UiCheckbox>
              {!activeFormats.length ? (
                <p className="!rounded-xl !bg-[var(--crm-yellow-soft)] !p-3 !text-sm !font-semibold !text-[var(--crm-yellow)]">
                  Crea al menos un formato en Productos → Formatos antes de
                  crear el producto.
                </p>
              ) : null}
              <div className="!grid !gap-2">
                {(advanced ? variantDrafts : variantDrafts.slice(0, 1)).map(
                  (variant) => (
                    <div
                      className="!grid !gap-2 sm:!grid-cols-[1fr_140px_auto_auto] sm:!items-center"
                      key={variant.id}
                    >
                      <CrmSelect
                        ariaLabel="Formato de venta"
                        onChange={(formatId) =>
                          updateDraft(variant.id, { formatId })
                        }
                        options={activeFormats.map((format) => ({
                          label: format.name,
                          value: format.id,
                          disabled: variantDrafts.some(
                            (draft) =>
                              draft.id !== variant.id &&
                              draft.formatId === format.id,
                          ),
                        }))}
                        placeholder="Selecciona un formato"
                        value={variant.formatId}
                      />
                      <UiInput
                        aria-label="Precio de variante"
                        className="h-11 min-h-11 w-full rounded-[var(--crm-radius-sm)] border border-transparent bg-[var(--crm-input-bg)] px-3.5 text-[13px] font-medium leading-[1.4] text-[var(--crm-text)] shadow-none outline-none transition-[border-color,box-shadow,background-color] duration-150 placeholder:text-[var(--crm-text-muted)] focus:border-[var(--crm-blue)] focus:shadow-[0_0_0_3px_var(--crm-blue-soft)] [&:is(textarea)]:h-auto [&:is(textarea)]:min-h-[88px] [&:is(textarea)]:resize-y [&:is(textarea)]:py-[11px]"
                        inputMode="decimal"
                        onChange={(event) =>
                      ...
                      <UiCheckbox
                        ...
                        className="!text-xs"
                        onChange={(checked) =>
                          updateDraft(variant.id, { active: checked })
                        }
                      >
                        Activa
                      </UiCheckbox>
                      <UiCheckbox
                        ...
                        className="!text-xs"
                        onChange={() =>
                          updateDraft(variant.id, { isDefault: true })
                        }
                      >
                        Predeterminada
                      </UiCheckbox>
                ...
                {advanced ? (
                  <UiButton
                    ...
                    disabled={variantDrafts.length >= activeFormats.length}
                    onClick={() => {
                      const used = new Set(
                        variantDrafts.map((variant) => variant.formatId),
                      );
                      const formatId =
                        activeFormats.find((format) => !used.has(format.id))
                          ?.id ?? "";
                      setVariantDrafts((current) => [
                        ...current,
                        {
              ...
              >
                Producto interno, sin aparición inicial en TPV
              </UiCheckbox>
              {!internal ? (
                <div className="!grid !gap-3 sm:!grid-cols-[1fr_1fr_auto] sm:!items-end">
                  <Field label="Pestaña inicial">
                    <CrmSelect
                      onChange={(value) => {
                        setTabId(value);
                        markDirty();
                  ...
                  <UiCheckbox
                    checked={initialPlacementFeatured}
                    className="!min-h-11 !text-xs !font-semibold"
                    onChange={(checked) => {
                      setInitialPlacementFeatured(checked);
                      markDirty();
                    }}
                  >
                    Destacado
          ...
          {formError ? (
            <p
              className="!rounded-lg !bg-red-500/10 !p-3 !text-sm !font-semibold !text-red-500"
              role="alert"
            >
              {formError}
            </p>
        ...
        {product ? (
          <>
            <section className="!grid !gap-3 !border-t !border-[var(--crm-border-subtle)] !pt-5">
              <div>
                ...
                <p className="!text-sm !text-[var(--crm-text-muted)]">
                  Cada variante utiliza uno de los formatos configurados para el
                  local.
                </p>
              </div>
              {productVariants.map((variant) => (
                <div
                  className="!grid !gap-2 !rounded-xl !bg-[var(--crm-surface-soft)] !p-3 sm:!grid-cols-[1fr_auto] sm:!items-center"
                  key={variant.id}
                >
                  {editingVariantId === variant.id ? (
                    <div className="!grid !gap-2 sm:!grid-cols-[minmax(180px,1fr)_140px_auto]">
                      <CrmSelect
                        ariaLabel="Formato de la variante"
                        onChange={setEditingVariantFormatId}
                        options={catalog.saleFormats.map((format) => ({
                          label: format.name,
                          value: format.id,
                          disabled:
                            !format.active ||
                      ...
                      <UiInput
                        ...
                        inputMode="decimal"
                        onChange={(event) =>
                          setEditingVariantPrice(event.target.value)
                        }
                        value={editingVariantPrice}
                      />
                      <UiButton
                        aria-label="Guardar variante"
                        className="inline-flex size-9 min-h-9 min-w-9 items-center justify-center gap-2 rounded-[9px] border-0 bg-[var(--crm-surface-soft)] p-0 text-xs font-semibold text-[var(--crm-text-secondary)] shadow-none transition-[background-color,color,transform] duration-150 hover:bg-[var(--crm-surface-hover)] hover:text-[var(--crm-text)] !bg-[var(--crm-blue)] !text-white"
                        disabled={disabled || !editingVariantFormatId}
                    ...
                    <div>
                      <strong>
                        {formatById.get(variant.formatId ?? "")?.name ??
                          variant.name}
                      </strong>{" "}
                  ...
                  <div className="flex min-w-0 items-center justify-end gap-[7px]">
                    <UiButton
                      ...
                      onClick={() =>
                        editingVariantId === variant.id
                          ? setEditingVariantId("")
                          : beginVariantEdit(variant.id)
                      }
                      title={
                        editingVariantId === variant.id ? "Cancelar" : "Editar"
                      }
                      type="button"
                    >
                      {editingVariantId === variant.id ? (
                    ...
                    <UiButton
                      ...
                      className="inline-flex size-9 min-h-9 min-w-9 items-center justify-center gap-2 rounded-[9px] border-0 bg-[var(--crm-surface-soft)] p-0 text-xs font-semibold text-[var(--crm-text-secondary)] shadow-none transition-[background-color,color,transform] duration-150 hover:bg-[var(--crm-surface-hover)] hover:text-[var(--crm-text)]"
                      disabled={
                        disabled || variant.isDefault || !variant.formatId
                      }
                      onClick={() =>
                        void mutate(() =>
                          catalogAdminService.updateVariant(catalog.venueId, {
                    ...
                    {!variant.isDefault ? (
                      <UiButton
                        className="inline-flex min-h-10 w-auto items-center justify-center gap-2 rounded-[var(--crm-radius-sm)] border-0 bg-[var(--crm-input-bg)] px-3.5 text-[13px] font-semibold leading-none text-[var(--crm-text-secondary)] shadow-none transition-[background-color,color,box-shadow,transform] duration-150 hover:bg-[var(--crm-surface-hover)] hover:text-[var(--crm-text)]"
                        disabled={disabled || !variant.active}
                        onClick={() =>
                          void mutate(() =>
                            catalogAdminService.setDefaultVariant(
                              catalog.venueId,
                              product.id,
                              variant.id,
                            ),
                    ...
                    <UiButton
                      ...
                      className="inline-flex size-9 min-h-9 min-w-9 items-center justify-center gap-2 rounded-[9px] border-0 bg-[var(--crm-surface-soft)] p-0 text-xs font-semibold text-[var(--crm-text-secondary)] shadow-none transition-[background-color,color,transform] duration-150 hover:bg-[var(--crm-surface-hover)] hover:text-[var(--crm-text)] inline-flex min-h-10 w-auto items-center justify-center gap-2 rounded-[var(--crm-radius-sm)] border-0 bg-[var(--crm-red-soft)] px-3.5 text-[13px] font-semibold leading-none text-[var(--crm-red)] shadow-none transition-[background-color,color,box-shadow,transform] duration-150 hover:brightness-95"
                      disabled={disabled || variant.isDefault}
                      onClick={() =>
                        void mutate(() =>
                          catalogAdminService.deleteVariant(
                            catalog.venueId,
                            product.id,
                            variant.id,
                          ),
              ...
              {!activeFormats.length ? (
                <p className="!rounded-xl !bg-[var(--crm-yellow-soft)] !p-3 !text-sm !font-semibold !text-[var(--crm-yellow)]">
                  No hay formatos activos. Créalo en Productos → Formatos.
                </p>
              ) : null}
              <div className="!grid !gap-2 sm:!grid-cols-[1fr_140px_auto]">
                <CrmSelect
                  ariaLabel="Nuevo formato de venta"
                  onChange={setNewVariantFormatId}
                  options={activeFormats.map((format) => ({
                    label: format.name,
                    value: format.id,
                    disabled: productVariants.some(
                      (variant) => variant.formatId === format.id,
                    ),
                  }))}
                  placeholder="Selecciona un formato"
                  value={newVariantFormatId}
                />
                <UiInput
                  aria-label="Precio de la nueva variante"
                  className="h-11 min-h-11 w-full rounded-[var(--crm-radius-sm)] border border-transparent bg-[var(--crm-input-bg)] px-3.5 text-[13px] font-medium leading-[1.4] text-[var(--crm-text)] shadow-none outline-none transition-[border-color,box-shadow,background-color] duration-150 placeholder:text-[var(--crm-text-muted)] focus:border-[var(--crm-blue)] focus:shadow-[0_0_0_3px_var(--crm-blue-soft)] [&:is(textarea)]:h-auto [&:is(textarea)]:min-h-[88px] [&:is(textarea)]:resize-y [&:is(textarea)]:py-[11px]"
                  inputMode="decimal"
                ...
                <UiButton
                  className="inline-flex min-h-10 w-auto items-center justify-center gap-2 rounded-[var(--crm-radius-sm)] border-0 bg-[var(--crm-input-bg)] px-3.5 text-[13px] font-semibold leading-none text-[var(--crm-text-secondary)] shadow-none transition-[background-color,color,box-shadow,transform] duration-150 hover:bg-[var(--crm-surface-hover)] hover:text-[var(--crm-text)]"
                  disabled={disabled || !newVariantFormatId}
            ...
            <section className="!grid !gap-3 !border-t !border-[var(--crm-border-subtle)] !pt-5">
              <div>
                <h3 className="!font-bold">Apariciones en TPV</h3>
                <p className="!text-sm !text-[var(--crm-text-muted)]">
                  El producto puede mostrarse varias veces y fijar una variante
                  distinta en cada ubicación.
                </p>
              </div>
              <div className="!grid !gap-2 sm:!grid-cols-[1fr_1fr_1fr_auto_auto] sm:!items-center">
                <CrmSelect
                  onChange={setPlacementTabId}
                  options={catalog.tabs
                    .filter((tab) => tab.active)
                    .map((tab) => ({ label: tab.label, value: tab.id }))}
                  value={placementTabId}
                />
                <CrmSelect
                  onChange={setPlacementCategoryId}
                  options={catalog.categories
                    .filter((category) => category.active)
                    .map((category) => ({
                      label: category.name,
                ...
                />
                <CrmSelect
                  onChange={setPlacementVariantId}
                  options={[
                  ...
                  value={placementVariantId}
                />
                <UiCheckbox
                  checked={placementFeatured}
                  className="!min-h-11 !text-xs !font-semibold"
                  disabled={disabled}
                  onChange={setPlacementFeatured}
                >
                  Destacado
                </UiCheckbox>
                <UiButton
                  aria-label={
                    placementEditId ? "Guardar aparición" : "Añadir aparición"
                  }
                  className="inline-flex size-9 min-h-9 min-w-9 items-center justify-center gap-2 rounded-[9px] border-0 bg-[var(--crm-surface-soft)] p-0 text-xs font-semibold text-[var(--crm-text-secondary)] shadow-none transition-[background-color,color,transform] duration-150 hover:bg-[var(--crm-surface-hover)] hover:text-[var(--crm-text)] !bg-[var(--crm-blue)] !text-white"
                  disabled={disabled || !placementTabId || !placementCategoryId}
              ...
              {productPlacements.map((placement) => (
                <div
                  className="!grid !gap-3 !rounded-xl !bg-[var(--crm-surface-soft)] !p-3 sm:!grid-cols-[minmax(0,1fr)_auto_auto] sm:!items-center"
                  key={placement.id}
                >
                  <span>
                    {
                      catalog.tabs.find((tab) => tab.id === placement.tabId)
                        ?.label
                    }{" "}
                    /{" "}
                    {catalog.categories.find(
                      (category) => category.id === placement.categoryId,
                    )?.name ?? "Sin categoría"}
                    {placement.pinnedVariantId
                      ? ` · ${productVariants.find((variant) => variant.id === placement.pinnedVariantId)?.name}`
                      : ""}{" "}
                    · {placement.active ? "Visible" : "Oculta"}
                  </span>
                  <UiCheckbox
                  ...
                  <div className="flex min-w-0 items-center justify-end gap-[7px]">
                    ...
                    <UiButton
                      ...
                      title={placement.active ? "Ocultar" : "Mostrar"}
                      type="button"
                    >
                      {placement.active ? (
                        <EyeOff className="!size-4" />
                    ...
                    <UiButton
                      aria-label="Eliminar aparición"
                      className="inline-flex size-9 min-h-9 min-w-9 items-center justify-center gap-2 rounded-[9px] border-0 bg-[var(--crm-surface-soft)] p-0 text-xs font-semibold text-[var(--crm-text-secondary)] shadow-none transition-[background-color,color,transform] duration-150 hover:bg-[var(--crm-surface-hover)] hover:text-[var(--crm-text)] inline-flex min-h-10 w-auto items-center justify-center gap-2 rounded-[var(--crm-radius-sm)] border-0 bg-[var(--crm-red-soft)] px-3.5 text-[13px] font-semibold leading-none text-[var(--crm-red)] shadow-none transition-[background-color,color,box-shadow,transform] duration-150 hover:brightness-95"
                      disabled={disabled}
                      onClick={() =>
                        void mutate(() =>
                          catalogAdminService.deletePlacement(
                            catalog.venueId,
                            placement.id,
                          ),
                        )
            ...
            <section className="!grid !gap-3 !border-t !border-[var(--crm-border-subtle)] !pt-5">
              <div>
                <h3 className="!font-bold">Mixers y modificadores asignados</h3>
                <p className="!text-sm !text-[var(--crm-text-muted)]">
                  Aplica el grupo a todo el producto o limita su alcance a una
                  variante concreta.
                </p>
              </div>
              <div className="!grid !gap-2 sm:!grid-cols-5">
                <CrmSelect
                  onChange={(value) => {
                    setAssignmentDomain(value as typeof assignmentDomain);
                    setAssignmentGroupId("");
                  }}
                  options={[
                    { label: "Mixers", value: "selection" },
                    { label: "Modificadores", value: "modifier" },
                  ]}
                ...
                <UiInput
                  ...
                  min={0}
                  onChange={(event) =>
                    setAssignmentMin(Number(event.target.value))
                  }
                  type="number"
                  value={assignmentMin}
                />
                <UiInput
                  ...
                  min={1}
                  onChange={(event) =>
                    setAssignmentMax(Number(event.target.value))
                  }
                  type="number"
                  value={assignmentMax}
                />
                <CrmSelect
                  onChange={setAssignmentVariantId}
                  options={[
              ...
              <UiButton
                className="inline-flex min-h-10 w-auto items-center justify-center gap-2 rounded-[var(--crm-radius-sm)] border-0 bg-[var(--crm-input-bg)] px-3.5 text-[13px] font-semibold leading-none text-[var(--crm-text-secondary)] shadow-none transition-[background-color,color,box-shadow,transform] duration-150 hover:bg-[var(--crm-surface-hover)] hover:text-[var(--crm-text)] !w-fit"
                disabled={disabled || !assignmentGroupId}
                onClick={() => void addAssignment()}
                type="button"
              >
                <Plus className="!size-4" /> Asignar grupo
              </UiButton>
              {productAssignments.map((assignment) => (
                <div
                  className="!flex !items-center !justify-between !gap-3 !rounded-xl !bg-[var(--crm-surface-soft)] !p-3"
                  key={`${assignment.domain}-${assignment.id}`}
                >
                  <span>
                    <strong>
                      {assignment.domain === "selection"
                        ...
                        : catalog.modifierGroups.find(
                            (group) => group.id === assignment.groupId,
                          )?.name}
                    </strong>{" "}
                    · {assignment.minSelection}–{assignment.maxSelection} ·{" "}
                    {assignment.appliesToAllVariants
                      ? "Todas las variantes"
                      : assignment.variantIds
                          .map(
                            (id) =>
