...
export function CatalogProductEditor({ catalog, defaultTaxRate, disabled, mutate, onClose, product }: Props) {
  ...
  const productAssignments = useMemo(() => [
    ...catalog.selectionAssignments
      .filter((assignment) => assignment.productId === product?.id && catalog.selectionGroups.some((group) => group.id === assignment.groupId && group.type === 'mixer'))
  ...
  ], [catalog.modifierAssignments, catalog.selectionAssignments, catalog.selectionGroups, product?.id])
  ...
  const assignmentGroups = assignmentDomain === 'selection' ? catalog.selectionGroups.filter((group) => group.type === 'mixer') : catalog.modifierGroups
      ...
      <div className="!grid !gap-5 !overflow-y-auto !p-5">
        ...
        {product ? (
          <>
            ...
            <ProductInventoryEditor catalog={catalog} disabled={disabled} product={product} />
