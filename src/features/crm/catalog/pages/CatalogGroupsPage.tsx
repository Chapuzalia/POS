  ...
  return (
    <div className="!grid !gap-4 xl:!grid-cols-[minmax(280px,.72fr)_minmax(0,1.28fr)]">
      <section className="min-w-0 overflow-hidden rounded-[var(--crm-radius-lg)] border-0 bg-[var(--crm-surface)] text-[var(--crm-text)] shadow-[var(--crm-shadow-card)] !overflow-hidden !rounded-2xl !bg-[var(--crm-surface)] !shadow-[var(--crm-shadow-card)]">
        <header className="!border-b !border-[var(--crm-border-subtle)] !p-5"><h2 className="!text-lg !font-bold">{domain === 'selection' ? 'Grupos de selección' : 'Grupos de modificadores'}</h2><p className="!text-sm !text-[var(--crm-text-muted)]">Reutilizables entre productos y variantes.</p><div className="!mt-4 !grid !gap-2"><UiInput className="h-11 min-h-11 w-full rounded-[var(--crm-radius-sm)] border border-transparent bg-[var(--crm-input-bg)] px-3.5 text-[13px] font-medium leading-[1.4] text-[var(--crm-text)] shadow-none outline-none transition-[border-color,box-shadow,background-color] duration-150 placeholder:text-[var(--crm-text-muted)] focus:border-[var(--crm-blue)] focus:shadow-[0_0_0_3px_var(--crm-blue-soft)] [&:is(textarea)]:h-auto [&:is(textarea)]:min-h-[88px] [&:is(textarea)]:resize-y [&:is(textarea)]:py-[11px]" onChange={(event) => setGroupName(event.target.value)} placeholder="Nombre del grupo" value={groupName} />{domain === 'selection' ? <CrmSelect onChange={(value) => setGroupType(value as typeof groupType)} options={[{ label: 'Componentes de menú', value: 'menu_component' }, { label: 'Mixer / acompañamiento', value: 'mixer' }]} value={groupType} /> : null}<UiButton className="inline-flex min-h-10 w-auto items-center justify-center gap-2 rounded-[var(--crm-radius-sm)] border-0 bg-[var(--crm-blue)] px-3.5 text-[13px] font-semibold leading-none text-white shadow-none transition-[background-color,color,box-shadow,transform] duration-150 hover:bg-[var(--crm-blue-hover)] hover:shadow-[0_8px_20px_rgba(20,120,237,0.22)] !w-fit" disabled={disabled || !groupName.trim()} onClick={() => void createGroup()} type="button"><Plus className="!size-4" /> Crear grupo</UiButton></div></header>
        <div className="!grid">
