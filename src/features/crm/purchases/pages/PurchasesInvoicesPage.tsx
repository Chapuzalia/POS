import { ScanLine, AlertTriangle, CheckCircle2, Clock3, Download, FileArchive, FilePlus2, Link2, RefreshCw, Search } from 'lucide-react'
...
export function PurchasesInvoicesCrm({ disabled, selectedVenueId, tenantContext }: Props) {
  ...
  const totalPages = Math.max(1, Math.ceil(filtered.length / CRM_PAGE_SIZE))
  const currentPage = Math.min(page, totalPages)
  const visibleDocuments = filtered.slice((currentPage - 1) * CRM_PAGE_SIZE, currentPage * CRM_PAGE_SIZE)
  const resetPage = (action: (value: string) => void) => (value: string) => { setPage(1); action(value) }
  ...
  return <section className="grid gap-5">
    <header className="flex flex-col gap-4 rounded-3xl bg-[var(--crm-surface)] p-5 shadow-[var(--crm-shadow-card)] lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-black uppercase tracking-widest text-[var(--crm-blue)]">Compras</p><h2 className="mt-1 text-2xl font-black">Facturas y albaranes</h2></div><div className="flex flex-wrap gap-2"><Button disabled={disabled || busy} onClick={() => setArchiveDocument(null)} type="button" variant="primary"><FilePlus2 className="size-4"/>Archivar documento</Button>{scanningEnabled ? <Button disabled={disabled || busy} onClick={() => setEditorDocumentId(null)} type="button" variant="secondary"><ScanLine className="size-4"/>Escanear documento</Button> : null}<Button disabled={busy || !exportable.length} onClick={() => void run(async () => { await exportPurchaseDocuments(exportable, startDate, endDate) })} type="button" variant="secondary"><FileArchive className="size-4"/>Exportar</Button></div></header>
    <div className="grid gap-3 rounded-3xl bg-[var(--crm-surface)] p-4 shadow-[var(--crm-shadow-card)] sm:grid-cols-2 lg:grid-cols-6"><label className="text-xs font-bold">Desde<Input type="date" value={startDate} onChange={(event) => { setPage(1); setStartDate(event.target.value) }}/></label><label className="text-xs font-bold">Hasta<Input type="date" value={endDate} onChange={(event) => { setPage(1); setEndDate(event.target.value) }}/></label><CrmSelect ariaLabel="Tipo" onChange={resetPage(setType)} options={[{ value: 'all', label: 'Todos los tipos' }, { value: 'invoice', label: 'Facturas' }, { value: 'delivery_note', label: 'Albaranes' }]} value={type}/><CrmSelect ariaLabel="Proveedor" onChange={resetPage(setSupplier)} options={[{ value: 'all', label: 'Todos los proveedores' }, ...suppliers.map((name) => ({ value: name, label: name }))]} searchable value={supplier}/><CrmSelect ariaLabel="Estado" onChange={resetPage(setStatus)} options={[{ value: 'all', label: 'Todos los estados' }, { value: 'processing', label: 'Procesando' }, { value: 'review', label: 'Pendiente de revisión' }, { value: 'confirmed', label: 'Confirmado' }, { value: 'error', label: 'Error' }]} value={status}/><label className="relative"><span className="sr-only">Buscar</span><Search className="pointer-events-none absolute left-3 top-3.5 size-4 text-[var(--crm-text-muted)]"/><Input className="!pl-9" placeholder="Número o proveedor" value={query} onChange={(event) => { setPage(1); setQuery(event.target.value) }}/></label></div>
    {error ? <p className="rounded-2xl bg-[var(--crm-red-soft)] p-4 text-sm font-semibold text-[var(--crm-red)]">{error}</p> : null}
    <div className="grid gap-3">{visibleDocuments.map((document) => <article className={`grid gap-4 rounded-3xl border-2 p-5 shadow-[var(--crm-shadow-card)] md:grid-cols-[1fr_auto] md:items-center ${statusPresentation[document.status].cardClassName}`} key={document.id}><button className="min-w-0 text-left" onClick={() => document.processingMode === 'archive' ? setArchiveDocument(document) : scanningEnabled ? setEditorDocumentId(document.id) : void run(() => downloadPurchaseOriginal(document))} type="button"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-[var(--crm-blue-soft)] px-2.5 py-1 text-xs font-black text-[var(--crm-blue)]">{document.documentType === 'invoice' ? 'Factura' : 'Albarán'}</span><span className="text-sm text-[var(--crm-text-muted)]">{document.documentDate ?? 'Fecha pendiente'}</span><PurchaseStatusBadge status={document.status}/>{document.linkedDocumentCount ? <span className="inline-flex items-center gap-1 text-xs text-[var(--crm-text-muted)]"><Link2 className="size-3"/>Vinculado</span> : null}</div><h3 className="mt-2 truncate text-lg font-black">{document.supplierName ?? 'Sin proveedor'}</h3><p className="mt-1 text-sm text-[var(--crm-text-muted)]">{document.documentNumber ?? 'Sin número'} · {document.affectsStock ? 'Stock actualizado' : 'Sin cambio de stock'}{document.excludedFromSpend ? ' · Gasto contabilizado por factura vinculada' : ''}</p></button><div className="flex items-center justify-between gap-3 md:justify-end"><strong className="text-xl">{document.processingMode === 'archive' || !scanningEnabled ? 'Archivo' : money(document.total)}</strong><Button aria-label="Descargar original" disabled={!document.storagePath || busy} onClick={() => void run(() => downloadPurchaseOriginal(document))} type="button" variant="tertiary"><Download className="size-4"/></Button></div></article>)}{filtered.length ? <CrmPagination currentPage={currentPage} onPageChange={setPage} totalResults={filtered.length} /> : null}{!filtered.length ? <div className="rounded-3xl bg-[var(--crm-surface)] p-10 text-center text-sm text-[var(--crm-text-muted)] shadow-[var(--crm-shadow-card)]">No hay documentos para los filtros seleccionados.</div> : null}</div>
  </section>
}
