        ...
        <div className="!min-w-[660px]">
          ...
          {units.map((unit) => {
            ...
            <div className="!grid !min-h-16 !grid-cols-[minmax(220px,1fr)_130px_190px_160px_120px] !items-center !gap-4 !border-b !border-[var(--crm-border-subtle)] !px-[22px] !py-3 !text-[13px]" key={unit.id}>
              ...
              <span className={unit.active ? 'crm-status-pill crm-status-pill-active !w-fit' : 'crm-status-pill crm-status-pill-inactive !w-fit'}>{unit.active ? 'Activa' : 'Inactiva'}</span>
            </div>
            )
          })}
        </div>
  ...
  return (
    <CrmModal label="Nueva unidad de inventario" onClose={onClose}>
      ...
      <form className="!grid !gap-4 !px-[22px] !py-5" onSubmit={(event) => { event.preventDefault(); void save() }}>
        <Field label="Nombre"><input autoFocus className="crm-input" maxLength={80} onChange={(event) => { setName(event.target.value); setValidationError(null) }} placeholder="Mililitros" value={name} /></Field>
        <Field label="Abreviatura"><input className="crm-input" maxLength={12} onChange={(event) => { setSymbol(event.target.value); setValidationError(null) }} placeholder="ml" value={symbol} /></Field>
        ...
        <button className="crm-primary-button !min-h-10 !rounded-[10px] !border-0 !bg-[var(--crm-blue)] !px-4 !font-semibold !text-white" disabled={disabled} type="submit">Crear unidad</button>
      </form>
    </CrmModal>
  )
}
