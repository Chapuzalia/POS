...
import { loadInventoryPreparationsPageData, saveInventoryProductionRecipe } from '../services/inventoryService'
...
export function InventoryPreparationsCrm({ disabled, runAction, selectedVenueId, tenantContext }: Props) {
  ...
  const refresh = useCallback(async () => setSnapshot({ ...emptySnapshot, ...(await loadInventoryPreparationsPageData(tenantContext, selectedVenueId)) }), [selectedVenueId, tenantContext])
  useEffect(() => { void runAction(refresh) }, [refresh, runAction])

  const preparationRecipeByItemId = useMemo(() => new Map(snapshot.productionRecipes.map((recipe) => [recipe.inventoryItemId, recipe])), [snapshot.productionRecipes])
  const productionLinesByRecipeId = useMemo(() => {
    const result = new Map<string, typeof snapshot.productionRecipeLines>()
    for (const line of snapshot.productionRecipeLines) result.set(line.recipeId, [...(result.get(line.recipeId) ?? []), line])
    for (const lines of result.values()) lines.sort((a, b) => a.sortOrder - b.sortOrder)
    return result
  }, [snapshot.productionRecipeLines])
  const itemsById = useMemo(() => new Map(snapshot.items.map((item) => [item.id, item])), [snapshot.items])
  const primaryRouteByItemId = useMemo(() => {
    const result = new Map<string, (typeof snapshot.itemRoutes)[number]>()
    for (const route of snapshot.itemRoutes) if (route.enabled && (!result.has(route.inventoryItemId) || route.priority < result.get(route.inventoryItemId)!.priority)) result.set(route.inventoryItemId, route)
    return result
  }, [snapshot.itemRoutes])
  ...
  function selectItem(nextId: string) {
    ...
    const recipe = preparationRecipeByItemId.get(nextId)
    setWarehouseId(recipe?.productionWarehouseId ?? primaryRouteByItemId.get(nextId)?.warehouseId ?? '')
    ...
    setLines(recipe ? (productionLinesByRecipeId.get(recipe.id) ?? []).map((line) => ({ inventoryItemId: line.inventoryItemId, quantity: String(line.quantity), unitId: line.unitId })) : [])
    setError(null)
  }
  ...
  return <section className="overflow-hidden rounded-2xl bg-[var(--crm-surface)] shadow-[var(--crm-shadow-card)]">
    ...
    <div className="grid gap-5 p-5 xl:grid-cols-[320px_1fr]">
      <div className="grid content-start gap-2"><h3 className="text-sm font-bold">Artículos elaborables</h3>{snapshot.items.map((item) => {
        const recipe = preparationRecipeByItemId.get(item.id)
