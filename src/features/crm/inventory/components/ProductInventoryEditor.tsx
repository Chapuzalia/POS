...
type Props = { catalog: CatalogData; disabled: boolean; product: CatalogProduct }
...
export function ProductInventoryEditor({ catalog, disabled, product }: Props) {
  ...
  useEffect(() => {
    ...
    const nextMode = !recipes.length ? 'none' : recipes.every((recipe) => recipe?.mode === 'direct') ? 'direct' : 'recipe'
    setMode(nextMode)
  ...
  }, [catalog.saleFormats, snapshot, variants])
  ...
  return <section className="grid gap-4 border-t border-[var(--crm-border-subtle)] pt-5">
    ...
    ] as const).map(([value, label]) => <button className={`rounded-xl border p-4 text-left font-semibold ${mode === value ? 'border-[var(--crm-blue)] bg-[var(--crm-blue-soft)] text-[var(--crm-blue)]' : 'border-[var(--crm-border-subtle)] bg-[var(--crm-surface-soft)]'}`} key={value} onClick={() => setMode(value)} type="button">{label}</button>)}</div>
