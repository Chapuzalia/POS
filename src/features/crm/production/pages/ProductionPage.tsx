...
export function ProductionCrm({ catalog, context, disabled, runAction, venueId }: Props) {
  ...
  const defaultPassId = state.passes.find((pass) => pass.isActive)?.id ?? ''
  ...
  return <div className="space-y-5">
    ...
    {state.venueEnabled ? <>
      ...
      <section className="rounded-2xl bg-[var(--crm-surface)] p-5 shadow-[var(--crm-shadow-card)]"><h2 className="text-lg font-black">Pases</h2><p className="mb-4 text-sm text-[var(--crm-text-muted)]">El destino indica dónde se prepara; el pase indica cuándo se envía. Producto prevalece sobre categoría. El pase por defecto recibe los productos y categorías sin pase asignado.</p>
        ...
        <div className="mb-5 space-y-2">{state.passes.map((pass) => <article className="flex items-center justify-between gap-2 rounded-xl border border-[var(--crm-border)] p-3" key={pass.id}><div className="flex min-w-0 flex-1 items-center gap-2"><Input defaultValue={pass.name} onBlur={(event) => { if (event.target.value.trim() && event.target.value !== pass.name) void mutate(() => saveProductionPass(context, venueId, { ...pass, name: event.target.value })) }} />{pass.id === defaultPassId ? <span className="shrink-0 rounded-full bg-[var(--crm-accent)] px-2 py-1 text-xs font-black text-[var(--crm-accent-foreground)]">Por defecto</span> : null}</div><div className="flex gap-1"><Button disabled={disabled || !pass.isActive || pass.id === defaultPassId} onClick={() => void mutate(() => setDefaultProductionPass(venueId, pass.id))} size="sm" type="button" variant="secondary">Por defecto</Button><Button disabled={disabled} onClick={() => void mutate(() => saveProductionPass(context, venueId, { ...pass, isActive: !pass.isActive }))} size="sm" type="button" variant="secondary">{pass.isActive ? 'Desactivar' : 'Activar'}</Button><Button disabled={disabled || pass.sortOrder === 0} onClick={() => void mutate(() => saveProductionPass(context, venueId, { ...pass, sortOrder: Math.max(0, pass.sortOrder - 1) }))} size="sm" type="button" variant="tertiary">↑</Button></div></article>)}</div>
