...
export function TableManagementPage({ context, disabled, venueId, onError }: Props) {
  ...
  const [selectedAreaId, setSelectedAreaId] = useState(''), [selectedTableId, setSelectedTableId] = useState<string | null>(null), [areaName, setAreaName] = useState(''), [busy, setBusy] = useState(false)
  const [guidelines, setGuidelines] = useState<Guidelines>({ x: null, y: null })
  const canvasRef = useRef<HTMLDivElement>(null), dragRef = useRef<DragState | null>(null)
  ...
  function startDrag(event: ReactPointerEvent, table: RestaurantTable, mode: DragState['mode']) {
    ...
    dragRef.current = { pointerId: event.pointerId, tableId: table.id, mode, startX: start.x, startY: start.y, initial: table, latest: table }; setSelectedTableId(table.id)
  }
  ...
  function drag(event: ReactPointerEvent) {
    ...
    event.preventDefault(); const point = screenToMap({ x: event.clientX, y: event.clientY }, canvas.getBoundingClientRect(), viewport), dx = point.x - state.startX, dy = point.y - state.startY
  ...
  return <div className="tables-admin">
    <section className="crm-panel tables-admin-settings"><div><h2>Gestion de mesas</h2><p>Activa el mapa y las comandas persistentes solo para este local.</p></div><label className="tables-toggle"><input checked={enabled} disabled={disabled || busy} onChange={(event) => { const value = event.target.checked; void run(async () => { await setVenueTablesEnabled(venueId, value); setEnabled(value) }) }} type="checkbox" /><span>{enabled ? 'Addon activado' : 'Addon desactivado'}</span></label></section>
    <section className="crm-panel tables-area-panel"><div className="crm-panel-header"><div><h2>Zonas</h2><p>Ordena y selecciona la zona que quieres editar.</p></div></div><form className="tables-area-create" onSubmit={(event) => { event.preventDefault(); void addArea() }}><input className="crm-input" onChange={(event) => setAreaName(event.target.value)} placeholder="Nueva zona" value={areaName} /><button className="crm-primary-button" disabled={disabled || busy || !areaName.trim()}><Plus size={16} /> Crear zona</button></form><div className="tables-area-list">{areas.map((area, index) => <div className={area.id === selectedAreaId ? 'tables-area active' : 'tables-area'} key={area.id} onClick={() => { setSelectedAreaId(area.id); setSelectedTableId(null) }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { setSelectedAreaId(area.id); setSelectedTableId(null) } }} role="button" tabIndex={0}><span>{area.name}</span><small>{tables.filter((table) => table.areaId === area.id && table.isActive).length} mesas</small><span className={area.isActive ? 'crm-status-pill crm-status-pill-active' : 'crm-status-pill crm-status-pill-muted'}>{area.isActive ? 'Activa' : 'Inactiva'}</span><span className="tables-order-actions"><button aria-label="Subir zona" disabled={index === 0 || busy} onClick={(event) => { event.stopPropagation(); void run(async () => { await Promise.all([updateDiningArea(context, area.id, { sortOrder: index - 1 }), updateDiningArea(context, areas[index - 1].id, { sortOrder: index })]); await refresh() }) }} type="button">↑</button><button aria-label="Bajar zona" disabled={index === areas.length - 1 || busy} onClick={(event) => { event.stopPropagation(); void run(async () => { await Promise.all([updateDiningArea(context, area.id, { sortOrder: index + 1 }), updateDiningArea(context, areas[index + 1].id, { sortOrder: index })]); await refresh() }) }} type="button">↓</button></span></div>)}</div></section>
    {selectedArea ? <section className="crm-panel tables-editor-panel"><div className="tables-editor-toolbar"><div><input aria-label="Nombre de zona" className="crm-input" onBlur={(event) => void run(() => updateDiningArea(context, selectedArea.id, { name: event.target.value }))} onChange={(event) => setAreas((current) => current.map((area) => area.id === selectedArea.id ? { ...area, name: event.target.value } : area))} value={selectedArea.name} /><button className="crm-state-button" onClick={() => void run(async () => { await updateDiningArea(context, selectedArea.id, { isActive: !selectedArea.isActive }); await refresh() })} type="button">{selectedArea.isActive ? 'Desactivar zona' : 'Activar zona'}</button></div><button className="crm-primary-button" disabled={disabled || busy} onClick={() => void addTable()} type="button"><Plus size={16} /> Nueva mesa</button></div>
      <div className="tables-editor-layout"><div className="tables-canvas" onPointerDown={viewportApi.startBackgroundPointer} onPointerMove={(event) => { drag(event); viewportApi.moveBackgroundPointer(event) }} onPointerUp={(event) => { finishDrag(event); viewportApi.endBackgroundPointer(event) }} onPointerCancel={(event) => { finishDrag(event); viewportApi.endBackgroundPointer(event) }} onWheel={viewportApi.onWheel} ref={canvasRef}><div className="map-transform-layer" style={{ transform: `translate(${viewport.panX}px, ${viewport.panY}px) scale(${viewport.zoom})` }}>{guidelines.x !== null ? <div className="tables-guideline vertical" style={{ left: `${guidelines.x}%` }} /> : null}{guidelines.y !== null ? <div className="tables-guideline horizontal" style={{ top: `${guidelines.y}%` }} /> : null}{areaTables.map((table) => <button className={`tables-canvas-table shape-${table.shape}${table.id === selectedTableId ? ' selected' : ''}${table.isActive ? '' : ' inactive'}`} key={table.id} onPointerDown={(event) => startDrag(event, table, 'move')} style={{ left: `${table.positionX}%`, top: `${table.positionY}%`, width: `${table.width}%`, height: `${table.height}%` }} type="button"><Armchair size={16} /><strong>{table.name}</strong><small>{table.capacity} pax</small><span className="tables-resize" onPointerDown={(event) => startDrag(event, table, 'resize')} /></button>)}{!areaTables.length ? <div className="tables-canvas-empty">Crea la primera mesa de esta zona.</div> : null}</div><MapViewportControls zoom={viewport.zoom} onFit={() => canvasRef.current && viewportApi.fit(canvasRef.current, areaTables)} onReset={() => viewportApi.setViewport({ zoom: 1, panX: 0, panY: 0 })} onZoomIn={() => canvasRef.current && viewportApi.zoomBy(1.2, canvasRef.current)} onZoomOut={() => canvasRef.current && viewportApi.zoomBy(1 / 1.2, canvasRef.current)} /></div>
      <aside className="tables-inspector">{selectedTable ? <><h3>{selectedTable.name}</h3><label>Nombre<input className="crm-input" onChange={(event) => patchSelectedTable({ name: event.target.value })} value={selectedTable.name} /></label><label>Capacidad<input className="crm-input" min="1" onChange={(event) => patchSelectedTable({ capacity: Number(event.target.value) })} type="number" value={selectedTable.capacity} /></label><label>Forma<select className="crm-input" onChange={(event) => patchSelectedTable({ shape: event.target.value as RestaurantTableShape })} value={selectedTable.shape}><option value="square">Cuadrada</option><option value="rectangle">Rectangular</option><option value="round">Redonda</option></select></label><div className="tables-inspector-actions"><button className="crm-save-button" disabled={busy} onClick={() => void saveSelectedTable()} type="button"><Save size={16} /> Guardar</button><button className="crm-secondary-button" disabled={busy} onClick={() => void addTable(selectedTable)} type="button"><Copy size={16} /> Duplicar</button><button className="crm-danger-button" disabled={busy} onClick={() => void run(async () => { await updateRestaurantTable(context, selectedTable.id, { isActive: !selectedTable.isActive }); await refresh() })} type="button"><Trash2 size={16} /> {selectedTable.isActive ? 'Desactivar' : 'Activar'}</button></div></> : <p>Selecciona una mesa para editar sus datos, moverla o cambiar su tamano.</p>}</aside></div>
    </section> : null}
  </div>
}
