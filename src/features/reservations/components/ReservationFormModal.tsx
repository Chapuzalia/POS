    ...
    <AppModal containerClassName="!max-w-3xl !p-0 sm:!p-4" dismissDisabled={props.disabled} label={props.reservation ? 'Editar reserva' : 'Nueva reserva'} onClose={props.onClose} placement="bottom">
      <section className="flex h-full w-[min(720px,100%)] flex-col overflow-hidden bg-[var(--surface)] text-[var(--foreground)] shadow-[var(--shadow)] [&_svg]:size-[18px] [&>header]:flex [&>header]:items-start [&>header]:justify-between [&>header]:gap-3 [&>header]:border-b [&>header]:border-[var(--separator)] [&>header]:p-[18px] [&_h2]:mt-[7px] [&_h2]:mb-0 [&_h2]:text-[22px] [&>header_p]:mt-[5px] [&>header_p]:mb-0 [&>header_p]:text-[var(--muted)] [&_label]:grid [&_label]:gap-1.5 [&_label]:text-[13px] [&_label]:font-extrabold [&_label]:text-[var(--foreground)] [&>footer]:flex [&>footer]:justify-end [&>footer]:gap-2.5 [&>footer]:border-t [&>footer]:border-[var(--separator)] [&>footer]:p-4 max-[760px]:[&>footer]:grid max-[760px]:[&>footer]:grid-cols-2">
        <header>
          ...
          <UiButton aria-label="Cerrar formulario" className="grid size-11 shrink-0 place-items-center rounded-[var(--radius)] border border-[var(--separator)] bg-[var(--surface)] text-[var(--foreground)] [&_svg]:size-[18px]" onClick={props.onClose} type="button"><X /></UiButton>
        </header>
        <div className="grid gap-4 overflow-auto p-[18px]">
          {lockedSchedule ? <div className="flex items-center gap-2 rounded-[var(--radius)] bg-[var(--accent-soft)] px-3 py-[11px] text-[13px] font-semibold">La reserva ya está sentada. Su horario y sus mesas no pueden modificarse.</div> : null}
          {validation ? <div className="flex items-center gap-2 rounded-[var(--radius)] bg-[var(--danger-soft)] px-3 py-[11px] text-[13px] font-semibold text-[var(--danger)]"><AlertTriangle /> {validation}</div> : null}
          {props.conflicts.length ? <div className="flex flex-col items-start gap-2 rounded-[var(--radius)] bg-[color-mix(in_srgb,var(--warning)_12%,var(--surface))] px-3 py-[11px] text-[13px] font-semibold text-[var(--warning)]">
          ...
          <div className="grid grid-cols-2 gap-3.5 max-[760px]:grid-cols-1">
            <label>Fecha *<UiInput disabled={lockedSchedule} onChange={(event) => setDate(event.target.value)} required type="date" value={date} /></label>
            <label>Hora *<UiInput disabled={lockedSchedule} onChange={(event) => setTime(event.target.value)} required type="time" value={time} /></label>
            <label>Duración *
              <UiNativeSelect disabled={lockedSchedule} onChange={(event) => setDuration(Number(event.target.value))} value={duration}>
                {durationOptions.map((minutes) => <option key={minutes} value={minutes}>{minutes} min</option>)}
              </UiNativeSelect>
            </label>
            <label>Personas *<UiInput min="1" onChange={(event) => setPartySize(Math.max(1, Number(event.target.value)))} required type="number" value={partySize} /></label>
            <label className="col-span-full max-[760px]:col-span-1">Nombre *<UiInput autoFocus required onChange={(event) => setCustomerName(event.target.value)} value={customerName} /></label>
            <label>Teléfono *<UiInput inputMode="tel" required onChange={(event) => setCustomerPhone(event.target.value)} value={customerPhone} /></label>
            <label>Email opcional<UiInput inputMode="email" onChange={(event) => setCustomerEmail(event.target.value)} type="email" value={customerEmail} /></label>
            <label className="col-span-full max-[760px]:col-span-1">Notas opcionales<UiTextArea onChange={(event) => setNotes(event.target.value)} rows={3} value={notes} /></label>
          </div>
          <section className="grid gap-3 border-t border-[var(--separator)] pt-4 [&>div:first-child]:flex [&>div:first-child]:items-center [&>div:first-child]:justify-between [&>div:first-child]:gap-3 [&_h3]:m-0 [&_h3]:text-[13px] [&_h3]:uppercase [&>div:first-child_span]:text-xs [&>div:first-child_span]:text-[var(--muted)]">
            ...
            {tableIds.length && selectedCapacity < partySize ? <div className="flex items-center gap-2 rounded-[var(--radius)] bg-[color-mix(in_srgb,var(--warning)_12%,var(--surface))] px-3 py-[11px] text-[13px] font-semibold text-[var(--warning)]"><AlertTriangle /> La capacidad seleccionada es inferior al número de personas.</div> : null}
        ...
        <footer>
          <UiButton className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[var(--radius)] border border-[var(--separator)] px-4 font-extrabold no-underline bg-[var(--surface)] text-[var(--foreground)]" onClick={props.onClose} type="button">Cancelar</UiButton>
          {props.conflicts.length ? <UiButton className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[var(--radius)] border border-[var(--separator)] px-4 font-extrabold no-underline border-[var(--accent)] bg-[var(--accent)] text-[var(--accent-foreground)]" disabled={props.disabled} onClick={() => void submit(true)} type="button">Guardar igualmente</UiButton>
            : <UiButton className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[var(--radius)] border border-[var(--separator)] px-4 font-extrabold no-underline border-[var(--accent)] bg-[var(--accent)] text-[var(--accent-foreground)]" disabled={props.disabled} onClick={() => void submit(false)} type="button">Guardar reserva</UiButton>}
        </footer>
      </section>
    </AppModal>
  )
