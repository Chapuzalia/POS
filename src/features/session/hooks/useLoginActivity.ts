...
export function useLoginActivity({ context, isOnline, onSessionClosed }: UseLoginActivityOptions) {
  ...
  useEffect(() => {
    ...
    const scheduleIdleClose = () => {
      ...
      idleTimeoutId = window.setTimeout(() => void close('La sesion se ha cerrado tras 4 horas sin actividad.', false), remainingMs)
    }
    const validateLease = async (forceHeartbeat = false) => {
      ...
      if (Date.now() - activity.lastActivityAt >= inactivityMs) {
        await close('La sesion se ha cerrado tras 4 horas sin actividad.', false)
        return
      }
      if (!isOnline) return
