...
Sentry.init({
  ...
  // Monitoriza aproximadamente el 5 % de las operaciones.
  tracesSampleRate: 0.05,

  // No grabar sesiones normales.
  replaysSessionSampleRate: 0,

  // Conservar el replay cuando sucede un error.
  ...
  beforeBreadcrumb(breadcrumb) {
    if (
      ...
      if (breadcrumb.data?.url) {
        breadcrumb.data.url = removeQueryString(
          breadcrumb.data.url,
        );
      }
    }
  ...
  beforeSend(event) {
    if (event.request) {
      ...
      if (event.request.url) {
        event.request.url = removeQueryString(
          event.request.url,
        );
      }

      if (event.request.headers) {
        event.request.headers = removeSensitiveHeaders(
