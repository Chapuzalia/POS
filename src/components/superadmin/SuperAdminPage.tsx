...
import { configureSuperadminFiscalEntity, createSuperadminFiscalEntity, loadSuperadminFiscalEntities, retrySuperadminFiscalEntity, updateSuperadminFiscalEntityVenues, type FiscalEntityOnboardingInput, type FiscalEntitySummary, type FiscalProvider } from '../../services/fiscalOnboardingService'
...
export function SuperAdminPage({ context, error, isOnline, onError, onLogout }: SuperAdminPageProps) {
  ...
  const [fiscalVenueEntity, setFiscalVenueEntity] = useState<FiscalEntitySummary | null>(null)
  const [editingFiscalVenueIds, setEditingFiscalVenueIds] = useState<string[]>([])
  ...
  async function openFiscalModal(tenant: PlatformTenant) {
    ...
    setFiscalVenueEntity(null)
    setEditingFiscalVenueIds([])
      ...
      {tenantModal?.mode === 'fiscal' ? (
        <SuperAdminModal label={`Fiscalidad de ${tenantModal.tenant.name}`} onClose={() => setTenantModal(null)} size="large">
          ...
          <div className="!grid !min-h-0 !gap-5 !overflow-y-auto !px-5 !py-5 sm:!px-6">
            ...
            {fiscalVenueEntity ? <div className="!grid !gap-3 !rounded-[12px] !border !border-[var(--crm-border)] !p-4"><h3 className="!m-0 !text-sm !font-bold">Editar locales</h3><p className="!m-0 !text-xs">Entidad: {fiscalVenueEntity.legalName}</p><fieldset className="!grid !gap-2"><legend className="!text-sm !font-bold">Locales asociados</legend>{tenantModal.tenant.venues.map((venue) => <UiCheckbox checked={editingFiscalVenueIds.includes(venue.id)} disabled={isBusy} key={venue.id} onChange={(checked) => setEditingFiscalVenueIds((current) => checked ? [...current, venue.id] : current.filter((id) => id !== venue.id))}>{venue.name}</UiCheckbox>)}</fieldset><div className="!flex !gap-2"><UiButton disabled={isBusy || !editingFiscalVenueIds.length} onClick={() => void runAction(async () => { const updated = await updateSuperadminFiscalEntityVenues(tenantModal.tenant.id, fiscalVenueEntity.id, editingFiscalVenueIds); setFiscalEntities((current) => current.map((item) => item.id === updated.id ? updated : item)); setFiscalVenueEntity(null); setSuccess('Locales de la entidad fiscal actualizados.') })} type="button">Guardar</UiButton><UiButton disabled={isBusy} onClick={() => setFiscalVenueEntity(null)} type="button">Cancelar</UiButton></div></div> : null}
