...
function defaultQuantities(
  ...
  return Object.fromEntries(
    sellable.selectionGroups.map((resolvedGroup) => [
...
function initialQuantities(
  ...
  for (const component of selection.components) {
    ...
    result[component.selectionGroupId] = {
      ...result[component.selectionGroupId],
      [component.id]: component.quantity,
    };
  }
  return result;
}
...
function defaultModifiers(
  ...
  return Object.fromEntries(
    groups.map((resolvedGroup) => [
      resolvedGroup.group.id,
      resolvedGroup.modifiers
...
function initialModifiers(
  groups: readonly ResolvedCatalogModifierGroup[],
  selection?: readonly TicketLineModifier[],
): SelectedModifiers {
  ...
  return Object.fromEntries(
    groups.map((resolvedGroup) => [
      resolvedGroup.group.id,
      selection
        .filter((modifier) => modifier.groupId === resolvedGroup.group.id)
        .map((modifier) => modifier.id),
    ]),
  );
}
...
function buildComponentModifierDefaults(
  ...
  for (const resolvedGroup of sellable.selectionGroups) {
    for (const option of resolvedGroup.options) {
      ...
      const component = resolveSellableProduct(
        catalog,
        option.product.id,
        option.variant.id,
      );
...
function initialComponentModifiers(
  ...
  if (!selection)
    return buildComponentModifierDefaults(catalog, sellable, quantities);
  return Object.fromEntries(
    selection.components.map((component) => [
      ...
      Object.fromEntries(
        [
          ...new Set(
            (component.modifiers ?? []).map((modifier) => modifier.groupId),
          ),
        ].map((groupId) => [
          groupId,
          (component.modifiers ?? [])
...
function buildSelection(
  ...
  const modifiers = sellable.modifierGroups.flatMap((resolvedGroup) =>
    resolvedGroup.modifiers
      .filter((modifier) =>
        selectedModifiers[resolvedGroup.group.id]?.includes(modifier.id),
      )
      .map((modifier) => ({
        id: modifier.id,
  ...
  const components = sellable.selectionGroups.flatMap((resolvedGroup) =>
    resolvedGroup.options
      .filter(
        (option) => (quantities[resolvedGroup.group.id]?.[option.id] ?? 0) > 0,
      )
      .map((option) => {
        const componentSellable = resolveSellableProduct(
          catalog,
          option.product.id,
          option.variant.id,
        );
        return {
          ...
          selectionGroupName:
            resolvedGroup.assignment.displayName ?? resolvedGroup.group.name,
          ...
          modifiers: componentSellable.modifierGroups.flatMap((modifierGroup) =>
            modifierGroup.modifiers
              .filter((modifier) =>
                selectedComponentModifiers[option.id]?.[
                  modifierGroup.group.id
  ...
  const mixer =
    components.find((component) => component.type === "mixer") ?? null;
  return {
    modifiers,
    components,
...
export function ProductDialog({
  ...
  const variants = useMemo(
    () =>
      catalog.variants
        .filter(
          (variant) => variant.productId === item.product.id && variant.active,
        )
        .sort(
          (left, right) =>
  ...
  const startsWithVariantSelection =
    !initialSelection && allowVariantSelection && variants.length > 1;
  ...
  const [selectedQuantities, setSelectedQuantities] =
    useState<SelectedQuantities>(initialQuantityState);
  const [selectedModifiers, setSelectedModifiers] = useState<SelectedModifiers>(
    () =>
      initialModifiers(
        selectedSellable.modifierGroups,
        initialSelection?.modifiers,
      ),
  );
  const [selectedComponentModifiers, setSelectedComponentModifiers] =
    useState<SelectedComponentModifiers>(() =>
      initialComponentModifiers(
        catalog,
        selectedSellable,
  ...
  const [hasChosenVariant, setHasChosenVariant] = useState(
  ...
  const mixerAssignment =
    selectedSellable.selectionGroups.find(
      (resolvedGroup) => resolvedGroup.group.type === "mixer",
    ) ?? null;
  const menuAssignments = selectedSellable.selectionGroups.filter(
    (resolvedGroup) => resolvedGroup.group.type === "menu_component",
  );
  const isChoosingMixer = !isChoosingVariant && Boolean(mixerAssignment);
  const selectedMixerItemId = mixerAssignment
    ? (Object.entries(selectedQuantities[mixerAssignment.group.id] ?? {}).find(
        ([, quantity]) => quantity > 0,
      )?.[0] ?? "")
    : "";
  const selectedMixerOption =
    mixerAssignment?.options.find(
      (option) => option.id === selectedMixerItemId,
    ) ?? null;
  const selectedMixerSellable = useMemo(
    () =>
      selectedMixerOption
        ? resolveSellableProduct(
            catalog,
            selectedMixerOption.product.id,
            selectedMixerOption.variant.id,
          )
        : null,
    [catalog, selectedMixerOption],
  );
  ...
  function resetVariantState(sellable: ResolvedSellableProduct) {
    ...
    setSelectedComponentModifiers(
      buildComponentModifierDefaults(catalog, sellable, quantities),
    );
  }

  function handleVariantSelect(variantId: string, sourceElement: HTMLElement) {
    const sellable = resolveSellableProduct(
      catalog,
      item.product.id,
      variantId,
    );
    ...
    if (!sellable.selectionGroups.length && !sellable.modifierGroups.length) {
      completeAdd(
        sellable,
        { modifiers: [], components: [], mixerProductId: null, mixer: null },
        sourceElement,
      );
    }
  }
  ...
  function setOptionQuantity(
    group: ResolvedCatalogSelectionGroup,
    option: ResolvedCatalogSelectionOption,
    quantity: number,
  ) {
    const currentGroup = selectedQuantities[group.group.id] ?? {};
    const otherCount = Object.entries(currentGroup).reduce(
      (total, [optionId, current]) =>
        total + (optionId === option.id ? 0 : current),
      0,
    );
    const maximum = Math.min(
      option.maxQuantity ?? group.assignment.maxSelection,
      group.assignment.maxSelection - otherCount,
    );
    ...
    if (nextQuantity > 0 && !selectedComponentModifiers[option.id]) {
      ...
      setSelectedComponentModifiers((current) => ({
        ...current,
        [option.id]: defaultModifiers(component.modifierGroups),
      }));
    }
  }
  ...
  function toggleOption(
    ...
    if (!selectedComponentModifiers[option.id]) {
      ...
      setSelectedComponentModifiers((currentModifiers) => ({
        ...currentModifiers,
        [option.id]: defaultModifiers(component.modifierGroups),
      }));
    }
  ...
  function handleMixerSelect(
    ...
    const nextQuantities = {
      ...selectedQuantities,
      [mixerAssignment.group.id]: { [option.id]: 1 },
    };
    const component = resolveSellableProduct(
      catalog,
      option.product.id,
    ...
    if (
      !selectedSellable.modifierGroups.length &&
      !menuAssignments.length &&
      !component.modifierGroups.length &&
      !initialSelection
    ) {
      const nextSelection = buildSelection(
        ...
        nextQuantities,
        selectedModifiers,
        nextComponentModifiers,
        undefined,
      );
      try {
        canonicalizeProductLineSelection(
          catalog,
          selectedSellable,
          nextSelection,
        );
        ...
        /* el boton final permite completar los modificadores requeridos */
      }
    }
  }
  ...
  function handleNoMixer(sourceElement: HTMLElement) {
    ...
    try {
      ...
      if (!selectedSellable.modifierGroups.length && !menuAssignments.length)
        completeAdd(selectedSellable, nextSelection, sourceElement);
    } catch {
      /* el botón final comunica que la selección sigue incompleta */
    }
  }

  function toggleModifier(
    group: ResolvedCatalogModifierGroup,
    modifierId: string,
    componentId?: string,
  ) {
    const update = (current: string[]) => {
      ...
      return group.assignment.maxSelection === 1
        ? [modifierId]
        : [...current, modifierId].slice(0, group.assignment.maxSelection);
    };
    if (componentId) {
      setSelectedComponentModifiers((current) => ({
        ...current,
        [componentId]: {
          ...current[componentId],
          [group.group.id]: update(
            current[componentId]?.[group.group.id] ?? [],
          ),
        },
      }));
    } else {
      setSelectedModifiers((current) => ({
        ...current,
        [group.group.id]: update(current[group.group.id] ?? []),
      }));
    }
  }
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="mt-5 grid gap-2">
            {variants.map((variant) => (
              <Button
                ...
                key={variant.id}
                onClick={(event) =>
                  handleVariantSelect(variant.id, event.currentTarget)
                }
                type="button"
                variant="tertiary"
              >
        ...
        {isChoosingMixer ? (
          <div className="mt-5">
            {mixerAssignment!.options.length ? (
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
                {mixerAssignment!.assignment.minSelection === 0 ||
                initialSelection ? (
                  <PosMixerCard
                    disabled={isBusy || hasSubmitted}
                    label="Sin mixer"
                ...
                {mixerAssignment!.options.map((option) => (
                  <PosMixerCard
                    ...
                    onSelect={(sourceElement) =>
                      handleMixerSelect(option, sourceElement)
                    }
            ...
            {selectedMixerOption &&
            selectedMixerSellable?.modifierGroups.length ? (
              <div className="mt-4 space-y-3 rounded-[var(--radius)] border border-[var(--separator)] p-3">
                {selectedMixerSellable.modifierGroups.map((group) => (
                  <div key={group.group.id}>
                    <p className="mb-2 text-sm font-semibold text-[var(--muted)]">
                      {group.assignment.displayName ?? group.group.name} ·{" "}
                      {group.assignment.minSelection}-
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                      {group.modifiers.map((modifier) => (
                        <Button
                          active={
                            selectedComponentModifiers[
                              selectedMixerOption.id
                            ]?.[group.group.id]?.includes(modifier.id) ?? false
                          }
                          disabled={hasSubmitted}
                          fullWidth
                          key={modifier.id}
                          ...
                          <span className="flex w-full items-center justify-between gap-3">
                            ...
                            <span>
                              {modifier.supplementCents
                                ? `+${formatMoney(modifier.supplementCents)}`
                                : "Incluido"}
                            </span>
                          </span>
                        </Button>
              ...
              <div key={group.group.id}>
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  {group.options.map((option) => {
                    const quantity =
                      selectedQuantities[group.group.id]?.[option.id] ?? 0;
                      ...
                      <div className="grid gap-2" key={option.id}>
                        ...
                        {selected && maximum > 1 ? (
                          <div className="flex items-center justify-center gap-2">
                            <Button
                              disabled={hasSubmitted || quantity <= 1}
                              onClick={() =>
                                setOptionQuantity(group, option, quantity - 1)
                              }
                              size="sm"
                              type="button"
                              variant="tertiary"
                            ...
                            <Button
                              disabled={hasSubmitted || quantity >= maximum}
                              onClick={() =>
                                setOptionQuantity(group, option, quantity + 1)
                              }
                              size="sm"
                        ...
                        {selected && component.modifierGroups.length ? (
                          <div className="rounded-[var(--radius)] border border-[var(--separator)] p-3">
                            {component.modifierGroups.map((modifierGroup) => (
                              <div
                                className="mb-3 last:mb-0"
                                key={modifierGroup.group.id}
                              >
                                <p className="mb-2 text-xs font-semibold text-[var(--muted)]">
                                  {modifierGroup.assignment.displayName ??
                                    modifierGroup.group.name}{" "}
                                  · {modifierGroup.assignment.minSelection}-
                                  {modifierGroup.assignment.maxSelection}
                                </p>
        ...
        {!isChoosingVariant && selectedSellable.modifierGroups.length ? (
          <div className="mt-5 space-y-4">
            {selectedSellable.modifierGroups.map((group) => (
              <div key={group.group.id}>
                <p className="mb-2 text-sm font-semibold text-[var(--muted)]">
                  {group.assignment.displayName ?? group.group.name}
                  {group.assignment.minSelection
                    ? ` - mínimo ${group.assignment.minSelection}`
                    : ""}
                </p>
                <div className="grid gap-2">
                  {group.modifiers.map((modifier) => (
                    <Button
                      active={
                        selectedModifiers[group.group.id]?.includes(
                          modifier.id,
                        ) ?? false
                      }
                      disabled={hasSubmitted}
        ...
        {!isChoosingVariant &&
        (selectedSellable.modifierGroups.length > 0 ||
          selectedSellable.selectionGroups.length > 0 ||
          initialSelection) ? (
          <div className="mt-5">
            <Button
              disabled={isBusy || hasSubmitted || !isSelectionValid}
              fullWidth
              onClick={(event) => submitSelection(event.currentTarget)}
              size="lg"
              type="button"
              variant="primary"
            >
